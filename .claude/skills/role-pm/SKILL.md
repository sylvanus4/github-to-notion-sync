...
name: role-pm
description: >-
  Analyze a given topic from the Product Manager perspective — PRD
  implications, sprint impact, user story generation, OKR alignment, roadmap
  changes, and stakeholder communication. Scores topic relevance (1-10) and
  produces a structured Korean analysis document when relevant (>= 5).
  Composes pm-execution, kwp-product-management-feature-spec,
  pm-product-discovery, kwp-product-management-roadmap-management,
  workflow-miner, skill-composer, and intent-alignment-tracker. Use when the
  role-dispatcher invokes this skill with a topic, or when the user asks for
  "PM perspective", "PM 관점", "기획자 분석", "product impact". Do NOT use for
  writing a full PRD (use pm-execution), sprint planning execution (use
  agency-sprint-prioritizer), or A/B test analysis (use pm-data-analytics).
