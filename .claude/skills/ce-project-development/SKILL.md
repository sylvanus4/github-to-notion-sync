...
description: >-
  Methodology for LLM-powered project development — Task-Model Fit
  recognition, manual prototype step, pipeline architecture, file system as
  state machine, structured output design, agent-assisted development, and
  cost estimation. Use when the user asks to "build LLM project", "design AI
  pipeline", "estimate AI costs", "manual prototype before automating", or
  mentions task-model fit, pipeline architecture, structured output, LLM
  project methodology, or agent-assisted development lifecycle. Do NOT use for
  general project planning (use full-stack-planner or sp-writing-plans). Do
  NOT use for existing pipeline execution (use mission-control or today). Do
  NOT use for prompt optimization (use prompt-architect). Korean triggers:
  "LLM 프로젝트 개발", "파이프라인 아키텍처", "태스크 모델 핏", "구조화 출력", "에이전트 개발 방법론".
