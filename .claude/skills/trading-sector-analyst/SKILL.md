...
description: >-
  Analyzes sector rotation patterns and market cycle positioning using
  TraderMonty's public CSV uptrend data. Use when the user asks to "run sector
  rotation analysis", "which sectors are leading", "cyclical vs defensive",
  "sectors overbought oversold", "market cycle phase", "섹터 로테이션", "시장 사이클",
  "과매수 과매도 섹터". Do NOT use for daily stock signals (use daily-stock-check). Do
  NOT use for weekly price updates (use weekly-stock-update).
---
