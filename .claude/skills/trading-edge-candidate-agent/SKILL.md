...
description: >-
  Generate and prioritize US equity long-side edge research tickets from EOD
  observations, then export pipeline-ready candidate specs for
  trade-strategy-pipeline Phase I. Use when the user asks to "convert
  hypotheses to research tickets", "discover edge candidates", "export
  strategy.yaml", "validate edge-finder-candidate", "에디지 후보 생성", "리서치 티켓 변환",
  or wants to turn anomalies into reproducible research tickets. Do NOT use
  for daily stock signals (use daily-stock-check). Do NOT use for AlphaEar
  signal tracking (use alphaear-signal-tracker).
---
