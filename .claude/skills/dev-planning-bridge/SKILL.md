...
description: >-
  개발 결과물에서 기획서 생성, 품질 점검, 문구 보강, 로컬 디자인 규칙 대조까지 End-to-End 역기획 파이프라인을
  오케스트레이션합니다. code-to-spec, spec-quality-gate, policy-text-generator, 로컬
  design-system 규칙(.cursor/rules/design-system.mdc), md-to-notion 스킬을 자동
  조합합니다. Use when the user asks to "역기획 파이프라인", "전체 역기획", "코드에서 기획서 전체 프로세스",
  "dev planning bridge", "full reverse spec pipeline", "개발 기획 브릿지",
  "end-to-end 역기획", "코드 분석부터 노션까지", or wants the complete pipeline from code
  ...
  policy-text-generator). Do NOT use for Figma or TDS-only design handoff
  (this repo uses Tailwind+Radix; use local
  `.cursor/rules/design-system.mdc`). Korean triggers: "역기획 파이프라인", "전체 역기획",
...
**Design SSOT:** Use **local** markdown rules at `.cursor/rules/design-system.mdc` (Tailwind + Radix). **No Figma URL** and **no TDS / `search_design_system`** in this pipeline.

## Pipeline architecture

```
Phase 1: code-to-spec              → Reverse spec (required) → local `docs/specs/`
Phase 2: spec-quality-gate         → Quality + auto-fix (required); align with POL-004
Phase 3: policy-text-generator     → Policy copy pass (optional)
Phase 4: design-system.mdc review  → Tailwind+Radix deltas vs spec (optional)
...
3. Cross-check narrative against **POL-004** (`docs/policies/04-document-quality-standards.md`) for planning docs in this repo
...
### Phase 4: Local design system linkage (optional)

Run only if `--skip-design` is **not** set and mode is `diff` or `today`:

1. Read **`.cursor/rules/design-system.mdc`** (Tailwind + Radix, Refined Swiss tokens) — do **not** call Figma MCP or `search_design_system` / TDS tools for this repo
2. Diff touched UI files vs rules: semantic colors, spacing, Radix primitives, chart styling
3. Append impact summary to the spec “changelog” section
4. Emit checklist of follow-ups for design alignment

**Skip**: `--skip-design`, `full` mode, or no UI/design-touched files in scope
...
| 4. design-system.mdc review | Done/Skipped | ~{N}s |
...
- [ ] Address {N} Tailwind/Radix deltas (vs design-system.mdc)
```
...
| Override | Path |
|----------|------|
| Design / UI (Tailwind + Radix) | [.cursor/skills/references/project-overrides/project-design-conventions.md](../references/project-overrides/project-design-conventions.md) |
| SSOT / doc pointers | [.cursor/skills/references/project-overrides/project-ssot.md](../references/project-overrides/project-ssot.md) |
| Document standards (POL-004) | [.cursor/skills/references/project-overrides/project-document-standards.md](../references/project-overrides/project-document-standards.md) |

**Constraints:** **No Figma**; **no TDS**; design system = **`.cursor/rules/design-system.mdc`**; specs live as **local markdown** under `docs/specs/` (and related `docs/`); quality expectations follow **POL-004** alongside `spec-quality-gate`.
