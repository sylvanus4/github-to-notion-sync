...
description: >-
  Systematic RCA for AI Platform Demo Pipeline Builder errors. Guides through
  KFP run status checks, block-level debugging via block_statuses and
  block_pods JSON parsing, "Run not found" handling, and per-block pod log
  retrieval. Use when the user asks to "debug pipeline", "pipeline error",
  "pipeline failed", "pipeline RCA", "파이프라인 에러", "파이프라인 장애", "파이프라인 원인 분석",
  "demo-pipeline-rca", "KFP error", "KFP run failed", "파이프라인 빌더 트러블슈팅", "block
  failed", "블록 실패", or encounters a failed/stuck pipeline in the AI Platform
  Demo environment. Do NOT use for Workload errors (use demo-workload-rca). Do
  NOT use for Serverless/Endpoint errors (use demo-serverless-rca). Do NOT use
