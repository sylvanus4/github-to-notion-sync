...
description: >-
  ...
  launch DAG-based agent teams, poll progress, and extract final reports. Use
  when the user asks to "run vibe swarm", "investment committee analysis",
  "multi-agent finance team", "vibe-trading swarm", "바이브 스웜", "멀티에이전트 분석",
  "투자위원회 분석", or wants collaborative AI agent research through Vibe-Trading's
  swarm system. Do NOT use for MiroFish simulations (use mirofish or
  mirofish-financial-sim). Do NOT use for project's role-dispatcher (use
  role-dispatcher). Do NOT use for setup (use vibe-trading-setup).
---
