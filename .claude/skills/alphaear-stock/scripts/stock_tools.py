...
KRX_PROXY_BASE = os.environ.get(
    "KSKILL_PROXY_BASE_URL", "https://k-skill-proxy.nomadamas.org"
)


class KRXProxy:
    """KRX(한국거래소) 종목 검색 및 시세 조회 via k-skill-proxy.

    사용자는 KRX_API_KEY 발급이 불필요하며 프록시가 키를 관리한다.
    read-only 일별 snapshot 전용 (실시간 호가/체결 미제공).
    """

    @staticmethod
    def _is_krx_query(query: str) -> bool:
        """한글이 포함되어 있거나 KRX 시장 키워드가 있으면 KRX 후보로 판단."""
        if re.search(r"[\uac00-\ud7a3]", query):
            return True
        upper = query.upper()
        if upper in ("KOSPI", "KOSDAQ", "KONEX"):
            return True
        return False
    ...
    @classmethod
    def search(cls, query: str, bas_dd: Optional[str] = None, limit: int = 10) -> List[Dict]:
        """종목명/코드로 KRX 상장 종목 검색."""
        params = {"q": query, "bas_dd": bas_dd or cls._today_yyyymmdd()}
        if limit:
            params["limit"] = str(limit)
        try:
            resp = _requests.get(
                f"{KRX_PROXY_BASE}/v1/korean-stock/search",
            ...
            logger.warning(f"⚠️ KRX search failed for '{query}': {e}")
            return []

    @classmethod
    def base_info(cls, market: str, code: str, bas_dd: Optional[str] = None) -> Dict:
        """종목 기본정보 조회."""
        params = {
            "market": market.upper(),
            "code": code,
            "bas_dd": bas_dd or cls._today_yyyymmdd(),
        }
        try:
            resp = _requests.get(
                f"{KRX_PROXY_BASE}/v1/korean-stock/base-info",
                params=params, timeout=10,
            )
            resp.raise_for_status()
            return resp.json().get("item", {})
        except Exception as e:
            logger.warning(f"⚠️ KRX base-info failed for {market}:{code}: {e}")
            return {}

    @classmethod
    def trade_info(cls, market: str, code: str, bas_dd: Optional[str] = None) -> Dict:
        """일별 시세(종가/거래량/시가총액 등) 조회."""
        params = {
            "market": market.upper(),
            "code": code,
            "bas_dd": bas_dd or cls._today_yyyymmdd(),
        ...
        try:
            ...
            logger.warning(f"⚠️ KRX trade-info failed for {market}:{code}: {e}")
            return {}

    @classmethod
    def trade_info_as_df(cls, market: str, code: str, bas_dd: Optional[str] = None) -> pd.DataFrame:
        """trade_info 결과를 표준 OHLCV DataFrame 1행으로 변환."""
        info = cls.trade_info(market, code, bas_dd)
        if not info:
            return pd.DataFrame()
        row = {
            "date": f"{info.get('base_date', '')[:4]}-{info.get('base_date', '')[4:6]}-{info.get('base_date', '')[6:8]}",
            "open": info.get("open_price"),
            "close": info.get("close_price"),
            "high": info.get("high_price"),
            "low": info.get("low_price"),
            "volume": info.get("trading_volume"),
            "change_pct": info.get("fluctuation_rate"),
        }
...
class StockTools:
    ...
    def search_ticker(self, query: str, limit: int = 5) -> List[Dict]:
        """
        模糊搜索股票代码或名称。
        KRX(KOSPI/KOSDAQ/KONEX) 한국 종목은 KRXProxy로 라우팅,
        A股/HK/US는 기존 로컬 DB 검색.
        """
        if KRXProxy._is_krx_query(query):
            krx_results = KRXProxy.search(query, limit=limit)
            if krx_results:
                return krx_results
    ...
    def _is_krx_ticker(ticker: str) -> bool:
        """KRX 티커인지 판별. .KS(KOSPI)/.KQ(KOSDAQ) suffix로 구분."""
        upper = ticker.upper()
        return upper.endswith(".KS") or upper.endswith(".KQ")

    @staticmethod
    def _krx_yfinance_ticker(code: str, market: str = "KOSPI") -> str:
        """KRX 종목코드 + 시장명을 yfinance ticker로 변환."""
        suffix = ".KQ" if market.upper() == "KOSDAQ" else ".KS"
        return f"{code}{suffix}"

    def get_krx_snapshot(self, code: str, market: str = "KOSPI", bas_dd: Optional[str] = None) -> Dict:
        """KRX 종목의 당일 시세 스냅샷 + 기본정보를 조합하여 반환."""
        base = KRXProxy.base_info(market, code, bas_dd)
        trade = KRXProxy.trade_info(market, code, bas_dd)
        return {**base, **trade}
    ...
    def get_stock_price(
        ...
        KRX 종목은 .KS(KOSPI) / .KQ(KOSDAQ) suffix 또는 search_ticker
        결과의 market 필드로 자동 라우팅되며 yfinance를 통해 조회한다.

        Args:
            ticker: 股票代码. A-share: "600519", US: "AAPL",
                    KRX: "005930.KS"(삼성전자) or "373220.KQ"(LG에너지솔루션).
            start_date: YYYY-MM-DD (default: 90d ago)
            end_date: YYYY-MM-DD (default: today)

        Returns:
            DataFrame with date, open, close, high, low, volume, change_pct.
        """
        now = datetime.now()
        if not end_date:
        ...
        is_krx = self._is_krx_ticker(ticker)
        cache_key = ticker.upper() if is_krx else ticker
        ...
        if need_update and is_krx:
            logger.info(f"📡 KRX ticker detected ({ticker}), fetching via yfinance...")
            try:
                yf_ticker = yf.Ticker(ticker.upper())
                end_dt = datetime.strptime(end_date, "%Y-%m-%d") + timedelta(days=1)
                df_krx = yf_ticker.history(start=start_date, end=end_dt.strftime("%Y-%m-%d"))
                if not df_krx.empty:
                    df_krx = df_krx.reset_index()
                    date_col = 'Date' if 'Date' in df_krx.columns else df_krx.columns[0]
                    df_krx = df_krx.rename(columns={
                        'Open': 'open', 'Close': 'close',
                        'High': 'high', 'Low': 'low', 'Volume': 'volume',
                    })
                    if pd.api.types.is_datetime64_any_dtype(df_krx[date_col]):
                        df_krx['date'] = df_krx[date_col].dt.strftime('%Y-%m-%d')
                    else:
                        df_krx['date'] = pd.to_datetime(df_krx[date_col]).dt.strftime('%Y-%m-%d')
                    df_krx['change_pct'] = df_krx['close'].pct_change().mul(100).fillna(0)
                    df_krx = df_krx[['date', 'open', 'close', 'high', 'low', 'volume', 'change_pct']]
                    self.db.save_stock_prices(cache_key, df_krx)
                    return self.db.get_stock_prices(cache_key, start_date, end_date)
                logger.warning(f"yfinance returned empty for KRX {ticker}")
            except Exception as e:
                logger.error(f"KRX yfinance fetch failed for {ticker}: {e}")
            return df_db
