...
description: >-
  Validate data quality in market analysis documents and blog articles before
  publication. Use when the user asks to "check data quality", "validate
  market data", "proofread financial doc", "데이터 품질 검사", "재무 데이터 검증", or needs
  validation of price scale consistency (ETF vs futures), instrument notation
  errors, date/day-of-week mismatches, allocation total errors, and unit
  mismatches. Supports English and Japanese content. Advisory mode — flags
  issues as warnings for human review, not blockers. Do NOT use for code
  quality review (use simplify or deep-review).
---
