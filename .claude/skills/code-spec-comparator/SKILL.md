...
description: >-
  ...
  find gaps, undocumented behavior, schema and state-transition mismatches,
  and spec-only items not present in code. Use when the user asks for "기획서
  싱크", "스펙 코드 비교", "code vs spec", "Notion 스펙 대조", "미구현 찾기", "문서화 안 된 API",
  "스키마 불일치", "역기획 대조", "gap analysis", "spec-code mismatch", "기획서대로 구현됐는지", or
  needs audit of implementation vs canonical planned spec. Do NOT use for
  first-time reverse spec from code only (use code-to-spec). Do NOT use for
  code review or security audit (use deep-review). Do NOT use for formal API
  doc generation (use technical-writer).
---
