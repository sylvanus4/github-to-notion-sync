...
description: >-
  ...
  "quality loop", "re-evaluate", "refine output", "improve quality
  iteratively", "품질 반복", "평가 최적화", or when first-draft quality consistently
  falls short. Do NOT use when first-draft quality already meets needs. Do NOT
  use for real-time tasks requiring immediate responses. Do NOT use when
  evaluation criteria are too subjective for consistent application. Do NOT
  use when deterministic tools (linters, type checkers, test runners) can
  check quality directly — use those instead.
