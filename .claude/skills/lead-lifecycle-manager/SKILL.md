...
description: >-
  Auto-registers leads from multiple channels (email, Slack, events), enriches
  with web research, classifies by segment, assigns owner, tracks dormancy,
  and auto-archives after 90 days idle. Use when "lead lifecycle", "리드 관리",
  "new lead intake", "dormant leads". Do NOT use for single lead research (use
  kwp-sales-account-research), CRM query (use Notion directly). Korean
  triggers: "리드 관리", "리드 라이프사이클", "신규 리드 접수".
