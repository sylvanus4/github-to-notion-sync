...
description: >-
  Set up Stripe payment processing — products, prices, checkout sessions,
  webhooks, subscriptions, and customer portal — with security best practices.
  Use when the user asks to "add payments", "set up Stripe", "integrate
  Stripe", "subscription billing", "payment flow", "webhook setup", "결제 연동",
  "스트라이프 설정", "구독 결제", "웹훅 설정", "결제 기능 추가", or needs to process payments via
  Stripe. Do NOT use for non-Stripe payment providers (PayPal, Square — handle
  directly), general accounting (use finance skills), or fraud analysis beyond
  Stripe Radar defaults.
