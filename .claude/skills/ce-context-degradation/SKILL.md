...
description: >-
  ...
  — lost-in-middle, context poisoning, distraction, confusion, and clash. Use
  when the user asks to "diagnose context problems", "fix lost-in-middle
  issues", "debug agent failures", "understand context poisoning", or mentions
  context degradation, attention patterns, context clash, context confusion,
  or agent performance degradation. Do NOT use for context compression
  strategies (use ce-context-compression). Do NOT use for Cursor-specific
  context compaction (use ecc-strategic-compact). Do NOT use for general bug
  diagnosis (use diagnose). Korean triggers: "컨텍스트 저하", "로스트 인 미들", "컨텍스트
  포이즌", "어텐션 패턴", "에이전트 성능 저하".
---
