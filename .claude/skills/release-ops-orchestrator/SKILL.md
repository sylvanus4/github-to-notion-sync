...
description: >-
  Master orchestrator for the weekly release operations loop with the Image
  Tagging Model. Routes to the correct phase skill based on the day of the
  week (Monday→reminder, Tuesday→collector+RC tagging, Wednesday→qa-gate,
  Thursday→deployer) and handles hotfix routing any day. Provides status
  overview and weekly lifecycle management. Use when the user asks to "run
  release ops", "release operations", "release-ops", "릴리즈 운영", "릴리즈 오케스트레이터",
