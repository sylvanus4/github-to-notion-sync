...
description: >-
  ...
  doc-review-orchestrator. Use when the user asks for "PRD lifecycle", "full
  PRD pipeline", "PRD end-to-end", "PRD 라이프사이클", "PRD 전체 파이프라인", "기획서 전체
  프로세스", or wants automated PRD creation through stakeholder review. Do NOT
  use for PRD creation only (use prd-research- factory). Do NOT use for doc
  quality check only (use doc-quality-gate). Do NOT use for simple PRD without
  research (use pm-execution create-prd).
---
