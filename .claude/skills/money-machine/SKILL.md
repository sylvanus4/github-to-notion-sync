...
name: money-machine
description: >-
  Master orchestrator that runs the full autonomous money-making pipeline
  end-to-end: data sync → stock screening → signal generation → content
  creation → content quality gate → content distribution → paper trading → P&L
  reporting → feedback loop. Composes `today`, `content-auto-gate`,
  `stock-content-
---
