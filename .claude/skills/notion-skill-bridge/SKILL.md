...
description: >-
  Compare Notion MCP-first workflows versus SKILL.md-first agent workflows for
  a given task; deliver a decision matrix (freshness, cost, reusability,
  constraint strength, collaboration, output stability) and a clear
  recommendation. Korean triggers: "노션 스킬 비교", "어떤 방식이 나아?", "노션 대 스킬".
  English triggers: "Notion vs Skill", "notion-skill-bridge", "MCP vs
  Skill.md", "workflow approach comparison", "Notion automation vs skill". Do
  NOT use for implementing integrations (write code/MCP config), arbitrary app
  comparisons unrelated to Notion or skills, or choosing unrelated tools (e.g.
  Jira).
---
