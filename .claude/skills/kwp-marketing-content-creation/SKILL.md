...
description: >-
  ...
  newsletters, landing pages, press releases, and case studies. Use when
  writing any marketing content, when you need channel-specific formatting,
  SEO-optimized copy, headline options, or calls to action. Do NOT use for
  tasks outside the marketing domain. Korean triggers: "최적화", "릴리즈", "이메일",
  "시장".
---
...
Before generating content, ask the user to clarify (use AskQuestion):

1. **Content type** — Blog post, social media (which platform?), email newsletter, landing page, press release, or case study?
2. **Target audience** — Who is this for? (role, seniority, industry, pain points)
3. **Key messages** — What 2-4 points must come through?
4. **Tone and length** — Any tone preference? Target word count or format constraints?

If the user has brand voice guidelines configured, note that they will be applied automatically. DO NOT start drafting until aligned on these inputs.
