...
description: >-
  ...
  per-domain adapters for trading, engineering, and PM reflection. Use when
  the user asks to "reflect", "run reflection", "daily reflection", "session
  reflection", "일일 회고", "리플렉션", "회고 실행", "오늘 돌아보기", "reflection-loop", or when
  invoked by daily-pm-orchestrator Phase 0. Do NOT use for engineering-only
  retrospectives with git metrics (use engineering-retro). Do NOT use for
  sprint retrospective-to-issue pipelines (use sprint-retro-to-issues). Do NOT
  use for inline post-task reflection during active work (follow
  post-task-reflection.mdc rule directly).
---
