...
name: cabinet
description: >-
  Set up, customize, and operate Cabinet — an AI-first knowledge base and
  startup OS built on Next.js 16 with markdown-on-disk architecture, AI agent
  workspaces, scheduled jobs, and wiki-style linking. Use when creating
  Cabinet instances, building agent templates, configuring job schedules,
  extending the editor, integrating with Cabinet's filesystem-based KB
  structure, or developing Cabinet plugins/apps. Do NOT use for general
  Next.js development without Cabinet context (use nextjs-best-practices). Do
  NOT use for generic markdown KB operations (use kb-orchestrator). Do NOT use
  for Obsidian vault management (use obsidian-* skills).
---
