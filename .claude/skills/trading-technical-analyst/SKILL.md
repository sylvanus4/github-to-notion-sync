---
name: trading-technical-analyst
description: >-
  Analyze weekly price charts for stocks, indices, cryptocurrencies, or forex
  pairs. Use when the user provides chart images and requests technical
  analysis, trend identification, support/resistance levels, scenario
  planning, or probability assessments based purely on chart data. Use for
  "technical analysis", "chart analysis", "지주 분석", "추세 분석", "지지/저항", "시나리오
  분석". Do NOT use for daily stock signals (use daily-stock-check). Do NOT use
  for AlphaEar analysis (use alphaear-deepear-lite). Do NOT use for weekly
  price updates (use weekly-stock-update).
---
...
**File Naming Convention**: Save each analysis as `outputs/reports/trading/[SYMBOL]_technical_analysis_[YYYY-MM-DD].md`

Example: `outputs/reports/trading/SPY_technical_analysis_2025-11-02.md`
...
6. Saves as outputs/reports/trading/SPY_technical_analysis_2025-11-02.md
```
...
3. Analyzes Bitcoin chart completely → Generates report → Saves as outputs/reports/trading/BTC_technical_analysis_2025-11-02.md
4. Analyzes Ethereum chart completely → Generates report → Saves as outputs/reports/trading/ETH_technical_analysis_2025-11-02.md
5. Analyzes Nasdaq chart completely → Generates report → Saves as outputs/reports/trading/NDX_technical_analysis_2025-11-02.md
...
### Example 1: Single chart analysis
**User:** "Please analyze this weekly chart of the S&P 500."
**Action:** Loads technical_analysis_framework.md, conducts systematic analysis (trend, S/R, MA, volume, patterns), develops 2–4 scenarios with probabilities.
**Output:** Report `SPY_technical_analysis_YYYY-MM-DD.md` with full analysis and scenario breakdown.

### Example 2: Breakout focus
**User:** "Will this stock break above resistance? Analyze the chart."
**Action:** Performs full analysis with emphasis on resistance levels, volume, and breakout vs rejection probability.
**Output:** Report with focused scenario analysis on breakout likelihood, invalidation levels, and key levels to watch.

### Example 3: Multiple charts
**User:** "Analyze these three charts: Bitcoin, Ethereum, and Nasdaq."
**Action:** Analyzes each chart sequentially; generates and saves separate report per symbol before proceeding.
**Output:** Three reports: `BTC_technical_analysis_YYYY-MM-DD.md`, `ETH_technical_analysis_YYYY-MM-DD.md`, `NDX_technical_analysis_YYYY-MM-DD.md`.

## Error Handling

| Error | Action |
|-------|--------|
| No chart image provided | Request user to upload or share chart image(s) |
| Chart unreadable (low resolution, truncated) | Request higher-resolution or full chart |
| Probabilities do not sum to 100% | Adjust scenario weights to sum to 100% before finalizing |
| Conflicting signals (e.g., bullish trend, bearish RSI) | Present both; note divergence and its implications |
