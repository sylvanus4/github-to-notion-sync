...
description: >-
  ...
  expanded test planning). Use when the user asks to "consensus plan",
  "ralplan", "multi-agent plan", "architect review plan", "critic review
  plan", "3-agent planning", "structured planning", "RALPLAN", "plan with
  consensus", "plan with review", "합의 기반 계획", "3에이전트 계획", "구조화된 계획", "합의 계획",
  "아키텍트 리뷰 계획", or needs a quality-gated planning process with architectural
  review and critical evaluation before execution. Do NOT use for simple
  single-step tasks (execute directly). Do NOT use for plan execution (use
  sp-executing-plans). Do NOT use for brainstorming without planning intent
  (use sp-brainstorming). Do NOT use for code review (use deep-review or
  simplify).
