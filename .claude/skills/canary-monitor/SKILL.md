...
name: canary-monitor
description: >-
  Post-deploy production monitoring loop that periodically captures
  screenshots, console errors, network failures, and performance metrics,
  comparing against a baseline. Uses 2-consecutive-confirmation before
  alerting to prevent false alarms. Use when the user asks for "canary
  monitor", "post-deploy check", "canary-monitor", "deploy monitoring", "배포 후
  모니터링", "카나리 모니터", "배포 확인", or wants post-deploy verification. Do NOT use for
  pre-merge validation (use release-commander), IaC review (use
  iac-review-agent), or infrastructure drift detection (use
