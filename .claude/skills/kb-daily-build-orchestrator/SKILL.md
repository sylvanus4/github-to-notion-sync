...
description: >-
  ...
  Triggers all 7 role-specific collector skills in parallel batches, then runs
  kb-compile and kb-index on updated topics. Single daily trigger builds the
  entire wiki knowledge system and posts a consolidated Korean intelligence
  report to Slack #효정-의사결정. Use when the user asks to "build daily KB", "run
  KB pipeline", "daily KB build", "위키 빌드", "일일 KB 빌드", "kb-daily-build",
  "/daily-kb-build", or when invoked by daily-pm-orchestrator. Do NOT use for
  individual collector runs (invoke kb-collect-* directly). Do NOT use for
  manual KB operations (use kb-orchestrator). Korean triggers: "일일 KB 빌드", "위키
  빌드", "KB 파이프라인".
