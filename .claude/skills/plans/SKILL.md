...
description: >-
  ...
  task delegation, dependency ordering, and verification criteria. Use when
  the user runs /plans, asks to "plan and optimize", "architect and plan",
  "optimize then plan", "프롬프트 최적화 후 계획", "플랜 작성", or wants both prompt
  optimization and execution planning in one flow. Do NOT use for prompt
  optimization only (use prompt-architect), execution only without
  optimization (use mission-control), or simple implementation planning (use
  /plan command).
---
