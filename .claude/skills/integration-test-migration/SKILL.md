...
description: >-
  Go 백엔드 통합 테스트를 마이그레이션 스키마 기반으로 전환합니다. DDL 안티패턴(DROP TABLE/CREATE TABLE) 제거,
  fixture INSERT 전환, mock 타입 정합, 상태 전이 정렬을 수행합니다. "통합 테스트 고도화", "DDL 제거",
  "integration test 마이그레이션 기반 전환", "테스트 스키마 정합성", "테스트 DDL 리팩토링" 요청 시 사용합니다.
  Do NOT use for 신규 테스트 작성 from scratch, gomock 단위 테스트 수정, 프론트엔드 테스트.
---
