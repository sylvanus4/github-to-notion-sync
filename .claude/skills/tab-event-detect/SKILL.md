---
name: tab-event-detect
description: >-
  Trigger event detection from RSS feeds via POST /events/detect. Use when
  detecting events, '이벤트 탐지', 'tab-event-detect', 'RSS event scan'. Do NOT use
  for manual event creation (use events API) or event study analysis (use
  tab-analysis-run).
---
