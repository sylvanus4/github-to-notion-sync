...
description: >-
  ...
  PagerDuty, and webhooks. Guides through trigger selection, tool
  configuration, MCP setup, memory management, and prompt writing. Use when
  the user asks to "create an automation", "set up a cron agent", "automate PR
  reviews", "schedule a daily digest", "cursor automation", "always-on agent",
  "자동화 설정", or any task involving recurring/event-driven cloud agents. Do NOT
  use for local CI checks (use ci-quality-gate), one-time cloud agent runs, or
  Cursor IDE hooks (use hooks configuration).
---
