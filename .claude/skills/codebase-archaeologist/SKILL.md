...
description: >-
  ...
  factor" reports, pattern evolution timelines, and dead code detection
  through commit frequency decay. Adds the TIME dimension to code
  understanding. Use when the user asks "who owns this code", "code
  archaeology", "bus factor", "churn analysis", "what's the riskiest module",
  "dead code", "코드 히스토리", "코드 소유자", "위험 분석", "who should review this", or any
  question about code ownership, history, or temporal risk patterns. Do NOT
  use for static code review (use simplify or deep-review), debugging (use
  diagnose), or git commit/PR operations (use domain-commit or ship).
---
