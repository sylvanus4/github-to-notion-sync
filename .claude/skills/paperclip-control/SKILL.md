...
description: >-
  ...
  "paperclip dashboard", "view costs", "paperclip approve", "페이퍼클립 상태", "회사
  목록", "에이전트 승인", "paperclip 비용", "예산 조회", "활동 로그", or any Paperclip
  governance and oversight task. Do NOT use for task/issue CRUD (use
  paperclip-tasks). Do NOT use for agent creation, heartbeats, agent budgets,
  or agent lifecycle (use paperclip-agents). Do NOT use for installation or
  deployment (use paperclip-setup).
