...
description: >-
  ...
  the user asks to "run in parallel", "fan out", "parallel workflow",
  "multiple perspectives", "concurrent agents", "parallel review", "병렬 실행",
  "동시 실행", or needs multiple independent analyses on the same input. Do NOT
  use for dependent tasks where order matters (use workflow-sequential). Do
  NOT use for iterative quality refinement (use workflow-eval-opt). Do NOT use
  when only 1 task exists.
---
