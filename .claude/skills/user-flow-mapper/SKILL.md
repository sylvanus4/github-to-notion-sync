...
description: >-
  Generate visual user-flow diagrams from PRDs, code, or specs: navigation,
  decisions, errors, and success paths. Output Mermaid diagrams, markdown
  flowcharts, or HTML via visual-explainer; cover happy path and key edge
  cases. Korean triggers: "사용자 플로우", "user flow", "플로우 다이어그램", "user flow
  mapper", "화면 흐름도", "flow diagram", "사용자 여정 맵", "유저 플로우". Do NOT use for
  market-level customer journey maps only (use pm-market-research journey
  tools), code reverse spec tables without diagrams (use code-to-spec), or
  interactive prototype building (manual design tools).
---
