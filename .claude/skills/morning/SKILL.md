...
name: morning
description: >-
  Morning routine orchestrator that runs google-daily (calendar briefing,
  Gmail triage, Drive upload, Slack notification) then today (stock data sync,
  screening, analysis, report) in sequence. Posts a consolidated morning
  briefing to Slack. Use when the user asks to "start my morning", "morning
  routine", "morning", "아침 루틴", "아침", "모닝", "하루 시작", "start the day",
  "/morning". Do NOT use for google-daily only (use google-daily), today
  pipeline only (use today), or end-of-day shipping (use eod-ship).
