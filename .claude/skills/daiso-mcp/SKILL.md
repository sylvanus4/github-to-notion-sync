...
name: daiso-mcp
description: >-
  Search stores, check product inventory, and look up movie seats across
  Daiso, Olive Young, and Megabox via the daiso-mcp MCP server. Use when the
  user asks to "find a Daiso store", "check Olive Young inventory", "search
  Daiso products", "Megabox movie seats", "근처 다이소", "올리브영 재고", "메가박스 영화", or
  any Korean retail store/inventory/movie lookup. Do NOT use for non-Korean
  retail services, general web scraping, or e-commerce purchasing.
---
