...
description: >-
  ...
  tooltips, errors, modals, empty states, CTAs, notifications, onboarding —
  with policy enforcement, cloud tone guides, and a 5-dimension rubric. Native
  English UI strings; Korean for analysis/reports unless copy is English. Use
  when: "write UI copy", "audit UI strings", "UX writing", "UX 라이팅", "에러 메시지",
  "툴팁", "모달", "빈 상태", "CTA", "톤앤보이스", "문구 일관성". Do NOT use for: ADRs/API
  docs/changelogs (technical-writer); prompts (prompt-transformer); decks
  (presentation-strategist); marketing/landing copy (pm-marketing-growth);
  brand voice guidelines (kwp-brand-voice-guideline-generation).
---
