...
description: >-
  ...
  a generic plugin template (replace ~~placeholders) or tweak an
  already-configured plugin. Guides through knowledge MCP search, todo-driven
  customization, MCP connector discovery, and packaging as a .plugin file. Use
  when the user asks to "customize plugin", "set up plugin", "configure
  plugin", "tailor plugin", or "update the standup command" for a Cowork
  plugin. Do NOT use for tasks outside the cowork domain.
---
...
Refer to the workflow steps above for usage patterns.
