---
name: trading-us-stock-analysis
description: >-
  Comprehensive US stock analysis including fundamental analysis (financial
  metrics, business quality, valuation), technical analysis (indicators, chart
  patterns, support/resistance), stock comparisons, and investment report
  generation. Use when the user asks to "analyze AAPL", "compare TSLA vs
  NVDA", "give me a report on Microsoft", "US stock analysis", "미국 주식 분석",
  "AAPL 재무 분석", "주식 비교", or requests investment recommendations for American
  stocks. Do NOT use for daily stock signals (use daily-stock-check). Do NOT
  use for AlphaEar analysis (use alphaear-deepear-lite). Do NOT use for weekly
...
### Example 1: Basic stock overview
**User:** "What's the current price of AAPL? Give me a quick overview."
**Action:** Web search for price, key metrics, 52-week range, recent news; presents concise summary.
**Output:** Company description, price, key metrics table, performance, notable news.

### Example 2: Full investment report
**User:** "Complete analysis of Microsoft. Should I invest?"
**Action:** Gathers fundamental and technical data, loads report-template.md and analysis references, synthesizes bull/bear cases, produces recommendation.
**Output:** Comprehensive report with executive summary, thesis, fundamental/technical sections, valuation, risk assessment, Buy/Hold/Sell rating, and target price.

### Example 3: Stock comparison
**User:** "Compare TSLA vs NVDA — which is better?"
**Action:** Fetches data for both tickers, creates side-by-side comparison (business, financials, valuation, technicals), identifies relative strengths.
**Output:** Comparison report with metrics tables, relative recommendation, allocation suggestion, and risk-adjusted assessment.

## Error Handling

| Error | Action |
|-------|--------|
| Ticker not found or invalid | Confirm ticker symbol; suggest correct exchange symbol (e.g., US vs foreign) |
| Stale or conflicting data | Prefer recent quarter; note data dates; cite sources |
| Missing financial data (private/small cap) | Note limited data; use available metrics; flag coverage gaps |
| Web search returns no useful results | Try alternate queries (company name, CUSIP); use SEC EDGAR directly if needed |
