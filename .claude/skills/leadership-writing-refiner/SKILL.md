...
description: >-
  ...
  clarify expectations, and insert structured growth guidance for juniors. Use
  when the user asks to "리더십 글 다듬기", "refine leadership", "팀 커뮤니케이션 개선", "글
  변환", "전달 방식 정제", "리더십 글 변환", "leadership tone refine", "리더십 톤 변환", "팀 피드백
  다듬기", "주니어 피드백 개선", or pastes text that needs leadership-aware rewriting. Do
  NOT use for general grammar-only fixes (use sentence-polisher). Do NOT use
  for full document rewriting without leadership context (use
  prompt-transformer). Do NOT use for brand voice enforcement (use
