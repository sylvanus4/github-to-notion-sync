...
description: >-
  Systematic RCA for AI Platform Demo Storage (Volume/Snapshot/Restore)
  errors. Guides through PVC/VolumeSnapshot lifecycle inspection, DB status
  queries, K8s resource checks, and a decision tree mapping storage states to
  root causes.
---
...
1. RCA 전체 내용(에러 요약 + 근본 원인 + 커밋 귀인 타임라인 + 해결 방안 + 재발 방지)을
   `outputs/demo-rca/{date}/volume-rca.md`에 저장
