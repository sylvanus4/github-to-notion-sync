...
description: >-
  Automatically detect and maintain bidirectional links between PRs and
  issues. On PR creation, find related issues from branch names, commit
  messages, and code changes. Update issues with PR status and auto-close on
  merge. Use when the user asks to "link PRs to issues", "bidirectional PR
  tracking", "PR-이슈 연결", "양방향 PR 추적", "pr-to-issue-linker", or wants automated
  PR-issue relationship management. Do NOT use for creating issues from
  commits (use commit-to-issue), PR review (use deep-review), or sprint triage
  (use sprint-orchestrator).
---
