...
description: >-
  Automatically extract decisions from meetings (meeting-digest output), Slack
  threads, and Notion discussions; log them to a Notion Decision Log database
  with context, rationale, alternatives, participants, and follow-ups; track
  status (proposed/approved/implemented/revised); notify Slack for new
  decisions and deadlines. Korean triggers: "의사결정 추적", "결정 이력", "의사결정 로그", "결정
  기록". English triggers: "decision tracker", "decision log",
  "decision-tracker". Do NOT use for generic meeting summaries without
  decision extraction (use meeting-digest), or task-only tracking without
  decision context (use meeting-action-tracker).
---
