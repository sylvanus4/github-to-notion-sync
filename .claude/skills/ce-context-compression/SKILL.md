...
description: >-
  Strategies for compressing agent context when sessions exhaust memory —
  anchored iterative summarization, opaque compression, regenerative
  summaries, artifact trail preservation, and probe-based evaluation. Use when
  the user asks to "compress context", "summarize conversation history",
  "implement compaction", "reduce token usage", or mentions context
  compression, structured summarization, tokens-per-task optimization, or
  long-running agent sessions exceeding context limits. Do NOT use for
  Cursor-specific context compaction (use ecc-strategic-compact). Do NOT use
  for general context optimization techniques (use ce-context-optimization).
  Do NOT use for memory system design (use ce-memory-systems or cognee).
  Korean triggers: "컨텍스트 압축", "대화 요약", "토큰 절감", "세션 압축", "컴팩션 전략".
---
