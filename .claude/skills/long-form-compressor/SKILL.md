...
description: >-
  Condense long documents into user-specified output formats (executive
  summary, bullet brief, one-page memo, tweet-length, custom word count) with
  configurable compression ratios and preservation priorities. Handles
  articles, reports, papers, transcripts, and multi-chapter documents. Use
  when the user asks to "summarize this", "compress this document", "make this
  shorter", "TL;DR", "executive summary", "one-page summary", "bullet point
  summary", "요약해줘", "짧게 줄여줘", "한 페이지로", "핵심만", "문서 압축", "긴 글 요약", or wants a
  document condensed to a specific length. Do NOT use for agent context
  compression (use ce-context-compression or ecc-strategic-compact). Do NOT
  use for prompt optimization (use prompt-architect). Do NOT use for meeting
  digest (use meeting-digest). Do NOT use for content repurposing across
