...
description: >-
  Build and orchestrate multi-agent TypeScript applications using the
  `open-multi-agent` framework (`@jackchen_me/open-multi-agent`). Provides
  three execution modes: single-agent runs (`runAgent`), auto-orchestrated
  team runs with coordinator decomposition (`runTeam`), and explicit task-list
  runs (`ru
