...
### Latest Model Mandate (최신 모델 필수 정책)

**원칙: 모델 성능 = 결과물 성능. 구세대 모델은 결과물 품질의 천장을 낮춘다.**

Jarvis가 계획을 수립하거나 스킬을 디스패치할 때, 생성형 AI API를 호출하는 모든 단계에서
**해당 시점의 최신 production-ready 모델**만 사용한다.

#### 적용 범위

| Domain | Mandate | Fallback |
|--------|---------|----------|
| Image generation | 현 시점 최신 (e.g. Imagen 4, gpt-image-2) | 직전 세대 1개만 허용 |
| Video generation | 현 시점 최신 (e.g. Veo 3.1, Sora) | Ken Burns/FFmpeg (quality 표기 필수) |
| LLM reasoning | 현 시점 최신 (e.g. Claude Opus 4.6, GPT-5.5) | 직전 세대 (비용 제약 시 명시) |
| Audio/TTS | 현 시점 최신 (e.g. Gemini 2.5 Flash TTS) | FFmpeg synthetic (quality 표기 필수) |
| Code generation | 현 시점 최신 tier의 coding model | 직전 세대 (예산 명시) |

#### 집행 규칙

1. **Plan 생성 시**: 각 step에서 사용할 구체적 모델명+버전을 명시 (e.g. `imagen-4.0-generate-001`, NOT `image model`)
2. **Skill dispatch 시**: 스킬 내부에서 구세대 모델을 호출하는 경우, Jarvis가 override 지시를 prompt에 삽입
3. **Fallback 사용 시**: fallback이 구세대 모델이면 반드시 `[QUALITY DEGRADED: using fallback model X due to Y]` 표기
4. **모델 discovery**: 불확실하면 WebSearch 또는 provider API (`models.list()`)로 최신 모델 확인 후 계획 수립
5. **비용 vs 품질 트레이드오프**: 비용 절감 목적으로 구세대 모델 선택 시 반드시 사용자에게 고지 + 승인 (`"구세대 모델 X 사용 시 ~30% 비용 절감, 품질 저하 감수합니까?"`)

#### 금지 패턴

- DALL-E 2 (DALL-E 3/gpt-image-2 존재 시)
- Veo 2.0 (Veo 3.1 존재 시)
- GPT-4o (GPT-5.5 접근 가능 시, 단 비용 제약 제외)
- Imagen 3 (Imagen 4 접근 가능 시)
- 문서나 스킬에 하드코딩된 구세대 모델명을 무비판적으로 따르는 행위

#### 위반 시 처리

- Plan review 단계에서 구세대 모델 감지 시 자동 경고 + 최신 대안 제시
- Goal Mode iteration 중 구세대 모델 사용 감지 시 다음 iter에서 교체 시도
