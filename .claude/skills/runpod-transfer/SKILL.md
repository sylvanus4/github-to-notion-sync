...
description: >-
  ...
  runpodctl's peer-to-peer transfer. Use when the user asks to "send files to
  RunPod", "download from RunPod", "transfer files", "upload dataset",
  "download checkpoint", "runpod send", "runpod receive", "runpod-transfer",
  "RunPod 파일 전송", "파일 보내기", "파일 받기", or any file transfer operation with
  RunPod. Do NOT use for installation or auth (use runpod-setup). Do NOT use
  for pod lifecycle management (use runpod-pods). Do NOT use for network
  volume CRUD (use runpod-volumes).
---
