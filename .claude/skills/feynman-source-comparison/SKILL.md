...
description: >-
  ...
  of agreements, disagreements, and confidence levels. Use when the user asks
  to 'compare papers', 'compare approaches', 'comparison matrix', 'compare
  tools', 'compare frameworks', 'cross-source analysis', 'agreement
  disagreement matrix', '논문 비교', '소스 비교', '도구 비교', '프레임워크 비교', '교차 분석',
  'feynman-source-comparison', '/compare', 'compare claims across sources'. Do
  NOT use for paper-code consistency checks (use feynman-paper-audit). Do NOT
  use for single-paper peer review (use feynman-peer-review). Do NOT use for
  competitive product analysis (use
  kwp-product-management-competitive-analysis or
  kwp-marketing-competitive-analysis). Do NOT use for general web research
  without structured comparison (use parallel-web-search).
---
