...
description: >-
  ...
  asks for "paper lifecycle", "full paper pipeline", "논문 라이프사이클", "논문 전체
  파이프라인", "paper end-to-end", or wants automated paper discovery through
  distribution. Do NOT use for paper review only (use paper-review). Do NOT
  use for slides generation only (use nlm-arxiv-slides). Do NOT use for
  related paper scouting only (use related-papers-scout).
---
