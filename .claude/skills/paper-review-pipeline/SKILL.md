...
description: >-
  End-to-end academic paper review pipeline that ingests a paper (arXiv URL,
  PDF, or markdown), produces a structured Korean review markdown, runs
  multi-perspective PM/research analysis using 7 PM skills, optionally
  generates a consolidated Word document and PowerPoint presentation (off by
  default, enable with --with-docx / --with-pptx), generates NotebookLM slide
  decks, creates structured Notion pages, and distributes everything to Slack
  with threaded summaries, Notion links, and file uploads. Use when the user
  asks to "paper review pipeline", "논문 파이프라인", "논문 분석 + 배포", "full paper
  pipeline", "paper-review-pipeline", "논문 리뷰 파이프라인", "논문 PM 분석", "논문 종합 분석 +
  슬랙", "논문 분석해서 노션이랑 슬랙에 올려줘", "논문 전체 파이프라인", "paper end-to-end", "논문 분석 + PM
  + 배포", or any request to systematically review and analyze an academic paper
  from multiple business/research perspectives AND distribute results to
  Notion/Slack. Do NOT use for paper analysis only without distribution (use
  paper-review). Do NOT use for arXiv-to-slides without review (use
  nlm-arxiv-slides). Do NOT use for general PDF reading (use anthropic-pdf).
  Do NOT use for market research without a paper (use pm-market-research). Do
  NOT use for ad-hoc Notion page creation (use Notion MCP directly). Do NOT
  use for peer review simulation with severity grading (use
  feynman-peer-review).
disable-model-invocation: true
---

# Paper Review Pipeline (End-to-End)
...
from different audience perspectives:

1. **초등학생용 (Elementary)** — Apply elementary-prompt.md system prompt, output Korean.
   누구나 쉽게 이해할 수 있도록 비유와 단순한 언어로 핵심을 설명.
2. **스티브잡스 발표자료용 (Steve Jobs Keynote)** — Apply expert-prompt.md system prompt, output Korean.
   깔끔한 흰 배경, 핵심 메시지 중심, 청중을 사로잡는 스토리텔링 구조.
...
Default channel: `#deep-research-trending`.

Use `slack_search_channels` MCP tool to resolve the channel ID.
...
1. Copy the review markdown to `outputs/papers/{date}/{slug}.md`.
...
| `--channel <name>` | Slack channel name for Phase 8 | `deep-research-trending` |
