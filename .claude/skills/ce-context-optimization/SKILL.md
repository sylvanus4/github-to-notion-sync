...
description: >-
  ...
  limits, observation masking, context budgeting, or extending effective
  context capacity. Do NOT use for context compression strategies (use
  ce-context-compression). Do NOT use for Cursor-specific context compaction
  (use ecc-strategic-compact). Do NOT use for token cost optimization in LLM
  API calls (use ecc-token-strategy rule). Do NOT use for context degradation
  diagnosis (use ce-context-degradation). Korean triggers: "컨텍스트 최적화", "KV
  캐시", "관찰 마스킹", "컨텍스트 파티셔닝", "토큰 비용 절감".
---
