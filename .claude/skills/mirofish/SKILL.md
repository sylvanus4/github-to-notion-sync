...
name: mirofish
description: >-
  Orchestrate MiroFish multi-agent swarm intelligence simulations —
  auto-generate seed documents from daily stock analysis, build GraphRAG
  knowledge graphs, run simulations with 26 AI agent personas, produce
  prediction reports, and interact with simulated agents. Supports a **Daily
  Stock Mode** for repeatable analysis from `today` pipeline outputs. Use when
  the user asks to "run MiroFish simulation", "predict scenario", "swarm
  simulation", "multi-agent prediction", "시뮬레이션 실행", "시나리오 예측", "MiroFish",
  "미로피쉬", "God's Eye injection", "digital sandbox", "/mirofish", or any
  request to simulate future scenarios using autonomous AI agents. Do NOT use
  for daily stock checks without simulation (use daily-stock-check). Do NOT
  use for simple financial news aggregation (use alphaear-news). Do NOT use
  for static technical analysis (use trading-technical-analyst).
---
...
Optionally post key predictions to Slack `#h-report` or `#deep-research-trending`.
