...
name: portal-scanner
description: >-
  Multi-strategy web scanner that extracts structured data from portals, job
  boards, product pages, or listing sites. Uses a 3-level content extraction
  cascade (Playwright browser → REST API → WebSearch fallback) with
  deduplication and scan history tracking.
---
