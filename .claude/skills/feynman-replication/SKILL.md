...
description: >-
  Plan or execute a replication of a paper, claim, or benchmark with
  environment selection (local, venv, Docker, HF Jobs, RunPod, Modal).
  Produces reproducible replication packages with scripts, data manifests, and
  result verification. Use when the user asks to 'replicate results',
  'reproduce an experiment', 'verify a claim empirically', 'build a
  replication package', 'experiment replication', 'reproduce this paper', '실험
  재현', '논문 재현', '결과 재현', '벤치마크 재현', 'feynman-replication', '/replicate',
  'replication plan'. Do NOT use for paper-code mismatch detection without
  execution (use feynman-paper-audit). Do NOT use for paper review (use
  feynman-peer-review). Do NOT use for model training (use hf-model-trainer).
  Do NOT use for running existing project backtests (use
