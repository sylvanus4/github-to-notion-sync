---
name: vllm-lmcache-integration
description: >-
  Reference architecture for integrating vLLM (inference engine) and LMCache
  (KV-cache persistence layer) with the LLM Wiki + MemKraft knowledge system.
  This is a **documentation-only** skill — the current system uses
  Cursor/Claude API for inference, not self-hosted vLLM. When self-hosted
  inference be
---
