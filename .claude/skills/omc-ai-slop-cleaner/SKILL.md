...
description: >-
  ...
  "consolidate duplicates", "AI 코드 정리", "AI 슬롭 제거", "코드 슬롭 정리", "중복 제거", "죽은
  코드 삭제", "불필요한 추상화 제거", or wants regression-safe cleanup of code that works
  but feels bloated, repetitive, or over-abstracted. Do NOT use for new
  feature builds or product changes. Do NOT use for broad redesigns (use
  deep-review). Do NOT use for formatting-only cleanup (use linters). Do NOT
  use when behavior is unclear and cannot be protected with tests.
---
