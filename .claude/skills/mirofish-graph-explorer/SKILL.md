...
description: >-
  Explore and query MiroFish GraphRAG knowledge graphs — browse entities,
  relationships, entity types, and graph topology built from seed documents.
  Use when the user asks to "explore graph", "browse knowledge graph", "show
  entities", "graph data", "entity relationships", "그래프 탐색", "지식 그래프", "엔티티
  관계", "mirofish graph", or any request to inspect the knowledge graph
  structure before or after simulation. Do NOT use for building graphs (use
  mirofish). Do NOT use for running simulations (use mirofish or
  mirofish-financial-sim). Do NOT use for general knowledge graph operations
  outside MiroFish (use cognee).
---
