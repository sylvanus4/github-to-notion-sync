...
description: >-
  Plan and manage US patent family strategies: continuation,
  continuation-in-part (CIP), divisional, and provisional applications.
  Advises on claim broadening, design-around prevention, picket-fence
  portfolios, and filing timeline management. Produces family tree
  visualizations and strategic filing recommendations. Use when the user asks
  to "plan patent family", "continuation strategy", "CIP analysis",
  "divisional filing", "patent portfolio strategy", "picket fence", or
  "maximize patent coverage". Do NOT use for Korean strategy (use
  patent-kr-strategy). Do NOT use for initial drafting (use
  patent-us-drafting). Do NOT use for OA response (use patent-us-oa-response).
  Korean triggers: "미국 특허 포트폴리오", "계속출원 전략", "CIP", "분할출원 (미국)", "특허 패밀리".
---
