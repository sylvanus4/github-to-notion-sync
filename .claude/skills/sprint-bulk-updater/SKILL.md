...
description: >-
  ...
  sprint assignments to the active iteration. Korean triggers: '스프린트 일괄 변경',
  '이번주 스프린트로 당겨줘', '스프린트 벌크 업데이트', '스프린트 당겨줘', '스프린트 정리', '일괄 스프린트', '스프린트
  통합', 'Todo 스프린트 변경', '미래 스프린트 당기기'. Do NOT use for single-issue sprint
  changes (use gh project item-edit directly). Do NOT use for creating new
  issues (use commit-to-issue). Do NOT use for sprint triage and priority
  assignment (use sprint-orchestrator). Do NOT use for status changes without
  sprint context. Do NOT use for cross-project bulk operations (this targets a
