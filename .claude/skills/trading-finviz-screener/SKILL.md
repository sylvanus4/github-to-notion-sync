...
description: >-
  Build and open FinViz screener URLs from natural language requests. Use when
  the user asks to "screen stocks", "find stocks matching criteria", "FinViz
  screener", "스크리너", "FinViz로 주식 필터", "高配当で成長している小型株を探したい", "Find oversold
  large caps with high ROE", or wants to filter by fundamentals or technicals.
  Supports Japanese and English input. No API key required for public
  screener. Do NOT use for daily stock signals (use daily-stock-check). Do NOT
  use for AlphaEar stock search (use alphaear-stock).
---
