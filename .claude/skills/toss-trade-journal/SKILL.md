...
description: >-
  Log executed trades with full context (signal source, entry reasoning,
  market conditions), track cumulative P&L, win rate, and strategy
  performance. Generate monthly trade journal reports as .docx via
  anthropic-docx. Use when the user asks to "log trade", "trade journal", "매매
  일지", "거래 기록", "trading performance", "승률 분석", "P&L report", or when invoked
  after tossinvest-trading completes an order. Do NOT use for placing orders
  (use tossinvest-trading). Do NOT use for portfolio snapshots (use
  toss-daily-snapshot). Do NOT use for risk assessment (use
  toss-risk-monitor).
