...
description: >-
  ...
  deduplication, classify each tweet by topic, and post to the appropriate
  Slack channel using the x-to-slack pipeline with rate limiting. Supports
  batch processing and resumable posting. Use when the user asks to "fetch
  tweets from timeline", "post timeline to slack", "twitter timeline", "scrape
  tweets", "twitter-timeline-to-slack", or wants to batch-process a user's
  tweets to Slack. Do NOT use for posting a single tweet URL (use x-to-slack).
  Do NOT use for general Slack messaging (use kwp-slack-slack-messaging). Do
  NOT use for Twitter scraping without Slack posting (use scrapling or
  agent-browser). Korean triggers: "타임라인", "트위터 타임라인", "트윗 일괄", "트윗 스크래핑".
---
...
```bash
# intel_registry.py is at scripts/intelligence/ in this repo
```
...
After updating tweets.json, save the intelligence artifact directly into the **internal** KB at `knowledge-bases/intelligence/` and register the URL in the central dedup registry.
...
2. Save and register via the internal registry script:
...
The script writes the artifact to `knowledge-bases/intelligence/raw/YYYY-MM-DD-{slug}.md`, updates `knowledge-bases/intelligence/url-registry.json`, and touches `.compile-pending` so the Stop hook auto-runs `kb_auto_compile.py intelligence` afterward.

The script is safe to call from any cwd inside this repo (it resolves paths relative to its own location). Slack post is already complete; failures here are non-blocking.
