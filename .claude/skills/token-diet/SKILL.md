...
description: >-
  ...
  token usage", "token diet", "diagnose tokens", "토큰 다이어트", "토큰 진단", "토큰 비용",
  "per-turn cost", "reduce token overhead", "token audit", "how much context
  am I using", or wants to understand and reduce their per-turn token
  footprint. Do NOT use for CLI output compression (handled by
