...
description: >-
  Create a new GitHub repository from the sylvanus4/cursor-template template,
  clone it to /Users/hanhyojung/thaki/, and open Cursor IDE. Use when the user
  asks to "create a new repo from template", "scaffold new project", "new repo
  from cursor-template", "template repo", "repo-from-template", "새 레포 만들어줘",
  "템플릿에서 레포 생성", "새 프로젝트 시작", or wants to bootstrap a new repository from the
  canonical Cursor template. Do NOT use for existing repo management (use
  github-workflow-automation), general git operations (use domain-commit),
  repo syncing across projects (use cursor-sync), or cloning arbitrary repos
  without template intent.
