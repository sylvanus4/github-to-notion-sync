...
> 💡 `--database-title`을 생략하면 자동으로 `"GitHub Sync - All Items - 타임스탬프"` 형식으로 생성됩니다.
...
```bash
# 특정 스프린트만 동기화 (데이터베이스명 자동 생성)
python scripts/create_new_database_sync.py \
  --parent-page-id 2939eddc34e68064b505c66d3c22b27a \
  --sprint-filter "25-10-Sprint4"

# 결과: "GitHub Sync - 25-10-Sprint4 - 20251021122345" 형식으로 생성됨
```

> 💡 Sprint 필터를 사용하면 자동으로 `"GitHub Sync - {스프린트명} - 타임스탬프"` 형식으로 생성됩니다.
...
| `--database-title` | ❌   | auto-generated | 새로 생성할 데이터베이스의 제목 (생략 시 자동 생성) |
...
- Sprint 필터 있음: `"GitHub Sync - {스프린트명} - {타임스탬프}"`
- Sprint 필터 없음: `"GitHub Sync - All Items - {타임스탬프}"`
- 타임스탬프 형식: `YYYYMMDDHHMMSS` (예: 20251021122345)
...
### 예시 1: 현재 Sprint 스냅샷 생성 (가장 일반적)

```bash
# Sprint 4의 모든 아이템을 별도 데이터베이스로 저장
# 데이터베이스명: "GitHub Sync - 25-10-Sprint4 - 20251021122345"
python scripts/create_new_database_sync.py \
  --parent-page-id 2939eddc34e68064b505c66d3c22b27a \
  --sprint-filter "25-10-Sprint4"
```
...
```bash
# 모든 아이템을 백업 (sprint 필터 없음)
# 데이터베이스명: "GitHub Sync - All Items - 20251021122345"
python scripts/create_new_database_sync.py \
  --parent-page-id 2939eddc34e68064b505c66d3c22b27a
```
...
python scripts/create_new_database_sync.py \
  --parent-page-id 2939eddc34e68064b505c66d3c22b27a \
  --sprint-filter "25-10-Sprint4" \
  --database-title "Sprint 4 최종 아카이브"
```

### 예시 4: Dry-run으로 사전 확인

```bash
# 1. Dry-run으로 확인 (실제 생성 안 함)
python scripts/create_new_database_sync.py \
  --parent-page-id 2939eddc34e68064b505c66d3c22b27a \
  --sprint-filter "25-10-Sprint4" \
  --dry-run
