...
Examples:
  ...
  python scripts/sprint_pr_review_check.py --team synos --sprint "25-07-Sprint4"
  ...
  python scripts/sprint_pr_review_check.py --sprint "25-07-Sprint4" --output pr_status.json --dry-run
    ...
    parser.add_argument(
        "--sprint",
        help="Sprint name (e.g., '25-07-Sprint4'). If using --team, defaults to team's current sprint."
    )
    parser.add_argument(
        "--notion-parent-id",
    ...
    if team_id:
        ...
        global GITHUB_TO_NOTION_NAME, GITHUB_TO_NOTION_USER_ID
        ...
        if not args.notion_parent_id:
            sprint_config = get_sprint_config_from_args(args)
            args.notion_parent_id = sprint_config.get("notion_parent_id")
            if args.notion_parent_id:
                logger.info(f"Using team's notion_parent_id: {args.notion_parent_id}")

    # Validate sprint is provided
    if not args.sprint:
