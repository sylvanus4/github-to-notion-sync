...
from src.models.webhook_models import WebhookParser, WebhookSignatureError, WebhookValidationError
...
class WebhookHandler:
    ...
    def verify_signature(self, signature: str, body: bytes) -> bool:
        ...
        expected_signature = hmac.new(self.settings.webhook_secret.encode("utf-8"), body, hashlib.sha256).hexdigest()
    ...
    async def handle_webhook(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            webhook_event = WebhookParser.create_webhook_event(event_type, delivery_id, signature, payload)
            ...
            logger.exception(f"Webhook validation failed: {e}")
            result["error"] = str(e)
            self.webhook_stats["total_failed"] += 1
    ...
    def _should_process_event(self, event_type: str, action: str | None) -> bool:
    ...
    async def validate_webhook_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result["success"] = (
                validation_result["webhook_secret_configured"] and validation_result["sync_service_ready"]
            )
    ...
    async def process_test_webhook(self, event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            webhook_event = WebhookParser.create_webhook_event(
                event_type, "test-delivery-id", "test-signature", payload
            )
            ...
            logger.info(
                ...
                extra={"event_type": event_type, "action": payload.get("action"), "success": result["success"]},
            )
...
def create_webhook_handler() -> WebhookHandler:
    ...
    logger.info("Webhook handler created", extra={"enabled_events": list(handler.config.webhook_events.keys())})
