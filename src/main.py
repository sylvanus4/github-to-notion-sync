...
def get_webhook_handler() -> WebhookHandler:
    ...
    if webhook_handler is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Webhook handler not initialized")
...
def get_sync_service() -> SyncService:
    ...
    if sync_service is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Sync service not initialized")
    return sync_service


# Webhook endpoints
@app.post("/webhook", response_model=WebhookResponse)
async def handle_webhook(request: Request, handler: WebhookHandler = Depends(get_webhook_handler)):
...
async def trigger_full_sync(
    ...
    try:
        ...
        return SyncResponse(success=True, message="Full sync started in background")

    except Exception as e:
        logger.exception(f"Error starting full sync: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to start full sync")


@app.post("/sync/cleanup", response_model=SyncResponse)
async def trigger_cleanup(background_tasks: BackgroundTasks, service: SyncService = Depends(get_sync_service)):
    ...
    try:
        ...
        return SyncResponse(success=True, message="Cleanup started in background")

    except Exception as e:
        logger.exception(f"Error starting cleanup: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to start cleanup")
...
async def health_check():
    ...
    return StatusResponse(status="healthy", timestamp=datetime.utcnow().isoformat(), version="1.0.0")


@app.get("/status", response_model=dict[str, Any])
async def get_status(
    handler: WebhookHandler = Depends(get_webhook_handler), service: SyncService = Depends(get_sync_service)
):
...
async def validate_configuration(handler: WebhookHandler = Depends(get_webhook_handler)):
...
async def reset_stats(handler: WebhookHandler = Depends(get_webhook_handler)):
    ...
    try:
        ...
        logger.exception(f"Error resetting stats: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to reset statistics")
...
async def get_app_config():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get configuration")
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        status_code=exc.status_code, content={"error": exc.detail, "timestamp": datetime.utcnow().isoformat()}
    )
