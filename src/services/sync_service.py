...
from src.models.github_models import GitHubProjectItem
from src.models.webhook_models import WebhookEvent, extract_project_item_from_webhook
...
class SyncService:
    ...
    async def _sync_single_item(self, item_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error syncing single item {item_id}: {e}")
            return False
    ...
    async def _archive_notion_item(self, item_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error archiving Notion item for GitHub ID {item_id}: {e}")
            return False

    async def full_sync(self, batch_size: int | None = None) -> dict[str, Any]:
    ...
    async def _process_batch(self, github_items: list[GitHubProjectItem]) -> dict[str, int]:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            # Submit all tasks
            future_to_item = {executor.submit(self._sync_item_to_notion, item): item for item in github_items}
    ...
    def _sync_item_to_notion(self, github_item: GitHubProjectItem) -> str:
        ...
        try:
            ...
            logger.exception(f"Error syncing item {github_item.id}: {e}")
            return "failed"
        ...
        try:
            ...
            for page in orphaned_pages:
                try:
                    ...
                    logger.exception(f"Error archiving orphaned page {page.id}: {e}")
                    result["failed"] += 1
    ...
    async def validate_sync_setup(self) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result["success"] = (
                validation_result["github_connection"]
                and validation_result["notion_connection"]
                and validation_result["field_mappings"]
            )
        ...
        if validation_result["success"]:
            ...
            logger.error("Sync setup validation failed", extra={"errors": validation_result["errors"]})
