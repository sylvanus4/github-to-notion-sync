...
- **팀별 독립 설정**: 각 팀이 자체 GitHub Project와 Notion 페이지로 동기화
- **유연한 설정 구조**: `config/teams/{team_id}/` 디렉토리 기반 관리
- **하위 호환성**: 기존 단일 팀 설정 계속 지원
- **CLI & 워크플로우 지원**: `--team` 옵션 및 GitHub Actions 팀 선택

### 📅 Daily Sprint Sync 자동화

- **Complete Resync**: GitHub Projects → Notion DB 전체 동기화
- **PR Review Check**: 스프린트 PR 리뷰 상태 체크
- **Sprint Stats**: 사용자별 이슈/PR/리뷰 통계
- **Sprint Summary**: AI 기반 스프린트 요약 (Claude)
- **Daily Scrum**: AI 기반 일일 스크럼 요약
...
print(f'Team: {config.team_name}, Sprint: {config.current_sprint}')
"
```
...
├── 🔄 .github/workflows/
│   └── daily-sprint-sync.yml           # 📅 Daily Sprint Sync 워크플로우
├── 🎯 scripts/
│   ├── common/
│   │   └── team_args.py                # 🏢 팀 인자 파서 (NEW!)
│   ├── complete_resync.py              # 🚀 전체 동기화
│   ├── daily_scrum_sync.py             # 📅 데일리 스크럼 (AI)
│   ├── sprint_summary_sync.py          # 📊 스프린트 요약 (AI)
│   ├── sprint_stats.py                 # 📈 스프린트 통계
│   ├── sprint_pr_review_check.py       # ✅ PR 리뷰 체크
│   └── migrate_to_multi_team.py        # 🔄 마이그레이션 스크립트
├── 📋 docs/
│   ├── daily-sprint-sync/              # 📅 Daily Sprint Sync 문서 (NEW!)
│   │   ├── README.md
│   │   ├── CONFIGURATION.md
│   │   ├── WORKFLOW-GUIDE.md
...
### Daily Sprint Sync 문서 (NEW!)

- 📅 **[Daily Sprint Sync 개요](docs/daily-sprint-sync/README.md)** - 시스템 개요
- ⚙️ **[설정 가이드](docs/daily-sprint-sync/CONFIGURATION.md)** - 팀별 설정 방법
- 🔄 **[워크플로우 가이드](docs/daily-sprint-sync/WORKFLOW-GUIDE.md)** - GitHub Actions 사용법
- 📜 **[스크립트 레퍼런스](docs/daily-sprint-sync/SCRIPTS-REFERENCE.md)** - CLI 옵션 상세
- 🌍 **[환경 설정](docs/daily-sprint-sync/ENVIRONMENT-SETUP.md)** - 환경 변수 설정
- 🆘 **[문제 해결](docs/daily-sprint-sync/TROUBLESHOOTING.md)** - 트러블슈팅
...
- 🆘 **[일반 문제 해결](docs/TROUBLESHOOTING.md)** - 트러블슈팅
