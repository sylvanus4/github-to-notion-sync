...
description: >-
  ...
  maintenance windows and rollback procedures, and optionally schedules
  patch operations via approval gate. Tier 1 for assessment and planning
  ...
  compliance-governance, infra-runbook-generator, and evaluation-engine.
  Use when the user asks to check patch compliance, plan maintenance window,
  missing patches report, patch readiness assessment, security patch audit,
  or needs to plan OS/package patching for MSP-managed infrastructure. Do
  NOT use for applying patches directly (execute via SSM/OS Config with
  separate approval), security vulnerability scanning only (use
  security-expert), application-level dependency updates (use
  dependency-auditor), or incident investigation (use
  incident-triage-summarizer). Korean triggers: 패치 관리, 유지보수 계획,
  패치 준수, 보안 패치 감사, 메인터넌스 윈도우 계획.
metadata:
  author: "thaki"
  version: "1.0.0"
  category: "msp/reliability"
  approval_tier: "tier-1/tier-2"
  approval_spec: "docs/msp-skills/APPROVAL_BOUNDARY_SPEC.md"
  mutations:
    - "maintenance_window_scheduling (tier-2, approval-gated)"
  clouds:
    - "aws"
    - "gcp"
  composes:
