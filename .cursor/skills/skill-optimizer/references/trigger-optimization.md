...
[
  {
    "query": "I need to review my PR for security issues and check test coverage before merging",
    "should_trigger": true,
    "note": "multi-domain review — core use case"
  },
  {
    "query": "Write a Python fibonacci function with memoization",
    "should_trigger": false,
    "note": "simple coding task — unrelated domain"
  }
]
```
...
Queries must be realistic — concrete, specific, and detailed. Include file paths, personal context, casual speech, typos, abbreviations, and varied lengths. Focus on edge cases, not clear-cut examples.

**Bad**: `"Review this code"`, `"Optimize my skill"`, `"Run tests"`

**Good**: `"hey can you check if my new pr-review-captain skill actually triggers when someone says 'review my PR'? It used to work but after the last model update I think it's broken"`
...
| Description lacks concrete nouns | Add specific things the skill handles (components, file types, operations) |
| No trigger phrases | Add exact phrases users type ("review my code", "audit this skill") |
...
Anthropic observes that Claude tends to under-trigger skills. Descriptions should be slightly "pushy" — proactively claiming edge cases. Example:
