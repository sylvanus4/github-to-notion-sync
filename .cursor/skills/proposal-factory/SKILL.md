---
description: RFP intake → classify requirements → search past proposals via Cognee KG → generate proposal draft with case studies → approval flow via Notion → version tracking → pre-submission checklist. Use when "proposal factory", "제안서 생성", "RFP response", "create proposal". Do NOT use for simple document creation (use anthropic-docx), contract review only (use kwp-legal-contract-review). Korean triggers: "제안서 생성", "제안 팩토리", "RFP 응답".
---
