...
description: >-
  ...
  multi-client architectures. Use when the user asks to "build background agent",
  ...
  infrastructure, self-spawning agents, or remote coding environments. Do NOT
  use for local dev stack setup (use local-dev-runner or local-dev-setup). Do NOT
  use for Kubernetes deployment review (use sre-devops-expert). Do NOT use for
  general CI/CD pipeline design (use sre-devops-expert).
  Korean triggers: "호스팅 에이전트", "샌드박스 실행", "백그라운드 에이전트",
  "멀티플레이어 에이전트", "웜풀 전략".
metadata:
  upstream: "muratcankoylan/Agent-Skills-for-Context-Engineering/skills/hosted-agents"
