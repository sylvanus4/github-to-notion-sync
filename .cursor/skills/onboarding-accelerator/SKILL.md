...
- Prompt: Read and follow `.cursor/skills/deep-review/SKILL.md`. Scope: full project. Focus on architecture — identify component boundaries, data flow patterns, key abstractions, shared libraries, and design decisions. Return a structured architecture summary.
- Expected output: component list, dependency graph, key patterns, tech stack

**Agent 2: codebase-archaeologist (ownership analysis)**
- `subagent_type: generalPurpose`, `model: fast`, `readonly: true`
- Prompt: Read and follow `.cursor/skills/codebase-archaeologist/SKILL.md`. Run `--mode full` on the entire project. Return ownership map, bus factor, and top 10 churn hotspots.
...
- Prompt: Read and follow `.cursor/skills/design-architect/SKILL.md`. Produce a design system summary: color palette, typography, component library, layout patterns.
...
- Prompt: Read and follow `.cursor/skills/technical-writer/SKILL.md`. Using all analysis from Groups A and B, create:
...
- Prompt: Read and follow `.cursor/skills/docs-tutor-setup/SKILL.md`. Transform the generated docs into a StudyVault with concept notes, practice questions, and a learning dashboard.
...
- `readonly: true`
- Prompt: Read and follow `.cursor/skills/local-dev-runner/SKILL.md`. Document the exact steps to set up the local development environment. Include prerequisites, services, ports, common issues and solutions.
- Integrate into: `{output}/01-getting-started.md`

**Agent 8: sre-devops-expert (infrastructure overview)**
- `readonly: true`
- Prompt: Read and follow `.cursor/skills/sre-devops-expert/SKILL.md`. Produce a service topology document: all services, their roles, communication patterns, deployment targets, and monitoring endpoints.
