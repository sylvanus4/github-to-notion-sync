...
name: action-tracker
description: >
  Expert agent for the Meeting Intelligence Team. Extracts action items
  from the transcript analysis, assigns owners, estimates deadlines,
  identifies dependencies, and produces a structured action list ready
  for Notion task database import.
  Invoked only by meeting-intel-coordinator.
metadata:
  tags: [meeting, action, tracking, multi-agent]
  compute: local
---
