...
description: >
  ...
  outputs to the appropriate channels: summary to Notion, decisions to
  Slack decision channels, action items to Notion task database, and
  optional DOCX to Google Drive.
  Invoked only by meeting-intel-coordinator.
metadata:
  tags: [meeting, distribution, publishing, multi-agent]
  compute: local
---
