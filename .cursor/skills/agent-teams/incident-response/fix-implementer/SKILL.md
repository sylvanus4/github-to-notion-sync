...
description: >
  ...
  targeted fix based on the confirmed root cause hypothesis. Produces a
  fix plan with rollback strategy, then applies the minimal change needed
  to resolve the incident.
  Invoked only by incident-response-coordinator after RCA confidence gate.
metadata:
  tags: [incident, fix, implementation, multi-agent]
  compute: local
---
