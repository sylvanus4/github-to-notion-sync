...
description: >
  ...
  triage output to focus on affected components. Supports re-invocation
  with a specific gap list from the root cause analyzer.
  Invoked only by incident-response-coordinator.
metadata:
  tags: [incident, evidence, collection, multi-agent]
  compute: local
---
