...
description: "Structured game ideation using SDT, Flow State, and Bartle Player Types. Use when the user asks to 'brainstorm game ideas', 'ideate game concept', 'game-brainstorm', '게임 아이디어', '게임 브레인스토밍', '게임 컨셉 구상'. Do NOT use for refining an already-chosen concept (use game-studio-design-system). Do NOT use for project scaffolding (use game-studio-start). Do NOT use for implementation planning (use game-studio-dev-story)."
---
