...
description: "Simulate playtesting with 4 Bartle archetypes (Achiever, Explorer, Socializer, Casual) and produce a consolidated feedback report. Use when the user asks to 'playtest', 'simulate players', 'game-playtest-report', 'player feedback', '게임 플레이테스트', '플레이테스트 시뮬레이션', '플레이어 피드백'. Do NOT use for QA test cases (use game-studio-qa-plan). Do NOT use for balance verification (use game-studio-balance-check). Do NOT use for GDD authoring (use game-studio-design-system)."
---
