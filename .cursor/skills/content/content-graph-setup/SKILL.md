...
description: >-
  ...
  "create content skill graph", "initialize content engine", "scaffold
  content graph", "콘텐츠 그래프 설정", "콘텐츠 엔진 초기화",
  "콘텐츠 스킬 그래프 만들어줘", "content-graph-setup", "build my content
  brain", "set up content system", or wants to create the foundational
  knowledge structure for AI-powered content production. Do NOT use for
  producing content from an existing graph (use content-graph-produce).
  Do NOT use for calibrating brand voice on an existing graph (use
  content-graph-voice). Do NOT use for auditing an existing graph (use
  content-graph-audit). Do NOT use for evolving an existing graph (use
