...
version: 1.0.0
description: >
  Data-backed sprint retrospective facilitator that combines engineering metrics
  (via engineering-retro + github-sprint-digest) with structured facilitation
  frameworks, mood/sentiment tracking, SMART action items, and retro-over-retro
  trend analysis. Produces a comprehensive retro document with discussion prompts,
  data highlights, and tracked follow-ups.
tags: [retro, retrospective, sprint, facilitation, sentiment, action-items, agile]
triggers:
  ...
  - "회고 준비"
do_not_use:
  - "For git-only engineering metrics without facilitation (use engineering-retro)"
  - "For daily standup summaries (use standup-digest)"
  - "For weekly status reports (use weekly-status-report)"
  - "For meeting digest and action items from a transcript (use meeting-digest)"
  - "For code review (use deep-review or simplify)"
  - "For sprint triage and issue assignment (use sprint-orchestrator)"
composes:
  - engineering-retro
