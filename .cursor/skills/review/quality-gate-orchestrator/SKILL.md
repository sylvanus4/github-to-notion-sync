...
description: >-
  ...
  (security audit, dependency analysis, performance profiling), and merges
  all findings into a deduplicated PASS/FAIL dashboard. Use when the user
  asks for "quality gate", "full quality check", "security + quality scan",
  "pre-release quality", "품질 게이트", "보안 + 품질 검사", "전체 품질 체크",
  or wants a comprehensive code health assessment before shipping. Do NOT
  use for CI checks only (use ci-quality-gate). Do NOT use for security
  audit only (use security-expert). Do NOT use for the full release pipeline
  (use release-commander).
metadata:
  version: "1.0.0"
  tags: ["orchestrator", "review", "quality", "security", "harness", "pipeline-fanout"]
