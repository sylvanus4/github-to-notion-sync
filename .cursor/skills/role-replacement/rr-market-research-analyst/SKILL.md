...
version: 1.0.0
description: >-
  Role Replacement Case Study: Market Research Analyst / Quant Trader — orchestrates the
  full daily market intelligence lifecycle from data sync through multi-strategy backtesting,
  multi-agent debate, quality gate, and broker integration. Thin harness composing today,
  daily-strategy-engine, trading-agent-desk, ai-quality-evaluator, toss-ops-orchestrator,
  and pine-script-generator into a single market-analyst role pipeline with MemKraft-powered
  decision memory and adversarial quality validation.
tags: [role-replacement, harness, trading, market-research, quant]
triggers:
  - rr-market-research-analyst
  - market analyst replacement
  - quant trader automation
  - 마켓 리서치 대체
  - 퀀트 트레이더
  - 시장 분석 자동화
...
do_not_use:
  ...
  - Single stock lookup (use alphaear-stock)
  - AlphaEar news aggregation only (use alphaear-news)
  - Toss order execution without analysis (use tossinvest-trading)
  - Daily stock check without strategy cards (use daily-stock-check)
composes:
  - today
