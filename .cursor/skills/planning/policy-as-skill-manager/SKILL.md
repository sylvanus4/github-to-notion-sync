...
description: >-
  Convert canonical policy documents into enforceable SKILL.md assets: fetch policy
  text from Notion, structure constraints/triggers/checklists as a skill, version
  skills, and flag when source policy changes require regeneration.
  Korean triggers: "정책 스킬 변환", "정책 자동화", "정책 SKILL.md", "정책을 스킬로".
  English triggers: "policy to skill", "policy-as-skill", "convert policy",
  "policy skill file", "Notion policy to SKILL.md".
  Do NOT use for generating end-user UI copy from policy (use policy-text-generator),
  generic skill authoring without a policy source (use create-skill), or legal review.
metadata:
  version: "1.0.0"
  category: meta
  author: thaki
