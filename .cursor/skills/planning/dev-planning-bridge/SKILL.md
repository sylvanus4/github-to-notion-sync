...
description: >-
  개발 결과물에서 기획서 생성, 품질 점검, 문구 보강, 로컬 디자인 규칙 대조까지
  End-to-End 역기획 파이프라인을 오케스트레이션합니다. code-to-spec,
  spec-quality-gate, policy-text-generator, 로컬 design-system 규칙(.cursor/rules/design-system.mdc),
  ...
  policy-text-generator). Do NOT use for Figma or TDS-only design handoff (this
  repo uses Tailwind+Radix; use local `.cursor/rules/design-system.mdc`). Korean triggers: "역기획 파이프라인", "전체 역기획",
