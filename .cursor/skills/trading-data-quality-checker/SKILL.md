---
name: trading-data-quality-checker
description: >-
  Validate data quality in market analysis documents and blog articles before publication. Use when the user asks to "check data quality", "validate market data", "proofread financial doc", "데이터 품질 검사", "재무 데이터 검증", or needs validation of price scale consistency (ETF vs futures), instrument notation errors, date/day-of-week mismatches, allocation total errors, and unit mismatches. Supports English and Japanese content. Advisory mode — flags issues as warnings for human review, not blockers. Do NOT use for code quality review (use simplify or deep-review).
metadata:
  author: tradermonty
  version: "1.0.0"
  category: execution
...
- `--output-dir`: Directory for report output (optional; default: `outputs/reports/trading/`)
...
### Example 1: Pre-publication validation
**User:** "Check data quality in my weekly strategy blog before I publish it."
**Action:** Runs `check_data_quality.py` on the document, loads instrument and common-error references, and reviews findings by severity.
**Output:** Markdown and JSON reports with ERROR/WARNING/INFO findings, line references, and suggested corrections.

### Example 2: Specific check subset
**User:** "Validate only price scale and allocation totals in this report."
**Action:** Runs script with `--checks price_scale,allocations` to focus on those categories.
**Output:** Filtered report with only price-scale and allocation findings.

### Example 3: Year inference for ambiguous dates
**User:** "The doc has dates like 'Jan 15' without year. Validate it."
**Action:** Uses `--as-of 2026-02-28` or document title metadata to infer year for weekday validation.
**Output:** Date-weekday validation results using inferred year; notes inference source in report.

## Error Handling

| Error | Action |
|-------|--------|
| File not found | Verify `--file` path exists; use absolute or correct relative path from project root |
| Parse error in markdown | Check for malformed tables, unclosed code blocks, or invalid YAML frontmatter |
| Exit code 1 despite valid run | Indicates script failure (missing file, parse error); exit 0 with findings is normal |
| False positives in allocation check | Ensure section headings match "配分", "Allocation", or similar; random body percentages are excluded |
