...
description: |
  Join a Google Meet or Zoom call as a real-time AI video avatar via PikaStreaming. Handles avatar generation, voice cloning, context-aware system prompts, session management (join/leave), and automatic billing. Use when the user drops a Google Meet or Zoom link, asks to "join a meeting", "attend this call", "video meeting bot", "join Google Meet", "Zoom 참가", "화상 회의 참가", "미팅 참여", "AI 아바타 미팅", "pikastream meeting", or wants an AI avatar to represent them in a video call. Do NOT use for local video file generation (use pika-text-to-video). Do NOT use for post-meeting transcription without live participation (use transcribee). Do NOT use for meeting digest or analysis (use meeting-digest). Do NOT use for scheduling meetings (use smart-meeting-scheduler or gws-calendar).
metadata:
  author: pika-labs (adapted by thakicloud)
  version: 0.1.0
  category: pika
  license: Apache-2.0
  upstream: https://github.com/Pika-Labs/Pika-Skills/tree/main/pikastream-video-meeting
  cost: $0.275/min
---
