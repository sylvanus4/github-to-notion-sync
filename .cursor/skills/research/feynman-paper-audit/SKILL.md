...
description: "Compare a paper's claims against its public codebase to identify mismatches, omissions, and reproducibility risks. Spawns evidence-gathering and verification subagents for rigorous claim-code comparison. Use when the user asks to 'audit a paper', 'check code-claim consistency', 'verify reproducibility', 'find mismatches between paper and code', 'paper code audit', 'claim verification', '논문 코드 감사', '논문-코드 불일치', '재현성 검증', '클레임 검증', 'feynman-paper-audit', 'code vs paper', '/audit'. Do NOT use for general paper review or critique without code comparison (use feynman-peer-review). Do NOT use for paper summarization or reading (use alphaxiv-paper-lookup or paper-review). Do NOT use for comparing multiple papers against each other (use feynman-source-comparison). Do NOT use for experiment replication (use feynman-replication)."
---
