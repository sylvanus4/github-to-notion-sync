...
description: >-
  ...
  file uploads.
  Use when the user asks to "paper review pipeline", "논문 파이프라인",
  "논문 분석 + 배포", "full paper pipeline", "paper-review-pipeline",
  "논문 리뷰 파이프라인", "논문 PM 분석", "논문 종합 분석 + 슬랙",
  "논문 분석해서 노션이랑 슬랙에 올려줘", "논문 전체 파이프라인",
  "paper end-to-end", "논문 분석 + PM + 배포",
  or any request to systematically review and analyze an academic paper from
  multiple business/research perspectives AND distribute results to Notion/Slack.
  Do NOT use for paper analysis only without distribution (use paper-review).
  ...
  Do NOT use for peer review simulation with severity grading (use feynman-peer-review).
metadata:
  author: thaki
  version: 3.0.0
  category: research
