---
description: Scheduled pipeline: queries Notion pipeline DB, computes win rate, conversion rate, pipeline velocity, generates interactive HTML dashboard, posts to Slack, uploads to Google Drive. Use when "sales dashboard", "영업 대시보드", "pipeline analytics", "weekly sales report". Do NOT use for single deal analysis (use kwp-sales-account-research), manual report (use anthropic-docx). Korean triggers: "영업 대시보드", "세일즈 대시보드", "파이프라인 분석".
---
