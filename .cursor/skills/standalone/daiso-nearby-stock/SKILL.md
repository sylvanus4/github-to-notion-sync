...
description: >-
  ...
  recommendations. Use when the user asks "다이소 재고 확인", "근처 다이소 재고",
  "다이소 매장 재고", "다이소에 XX 있어?", "다이소 XX 파는 곳", "다이소 XX 재고 있는
  매장", "check Daiso stock nearby", or any request combining a Daiso product
  with location-based inventory lookup. Do NOT use for Olive Young, Megabox,
  or non-Daiso lookups (use daiso-mcp directly). Do NOT use for product
  search without inventory intent (use daiso-mcp daiso_search_products).
metadata:
  author: "thaki"
  version: "1.0.0"
  category: "execution"
