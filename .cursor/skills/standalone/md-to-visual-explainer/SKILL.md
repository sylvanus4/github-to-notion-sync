...
description: >-
  ...
  Mermaid diagrams, before/after comparisons, evidence sections, severity badges,
  and ELI5 (elementary school version) explanations for each item. Use when the
  user asks to "make this document visual", "explain visually", "시각적으로 설명",
  "이해하기 쉽게 HTML로", "초등학생 버전", "visual explainer", "쉽게 풀어서",
  "그림으로 설명", "다이어그램으로 설명", "문서 시각화", "리뷰 시각화",
  "md-to-visual-explainer", or wants a markdown document transformed into an
  interactive visual page for comprehension and review. Do NOT use for generating
  diagrams from scratch without a source document (use visual-explainer). Do NOT
  use for simple markdown-to-HTML conversion without visual enhancement. Do NOT
  use for slide deck generation (use nlm-slides or anthropic-pptx). Korean
  triggers: "시각적 설명", "문서 시각화", "쉽게 이해", "초등학생 버전", "그림으로".
---
