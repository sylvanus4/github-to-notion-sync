...
description: >-
  ...
  SVG rendering, page layout inspection, and structure dumps. Use when the user
  asks to "view HWP", "open HWP", "inspect HWP", "HWP 열기", "HWP 보기",
  "HWP 구조 확인", "rhwp-viewer", or wants to visually examine or inspect the
  structure of an HWP document. Do NOT use for converting HWP to other formats
  for pipeline use (use rhwp-converter). Do NOT use for systematic debugging
  with ir-diff (use rhwp-debug). Do NOT use for installing rhwp (use
  rhwp-setup).
metadata:
  author: "thaki"
  version: "1.0.0"
