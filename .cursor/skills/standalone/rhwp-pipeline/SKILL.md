...
description: >-
  ...
  HWP format, export as SVG/PDF, and distribute via Slack or archival. Integrates
  with existing pipeline skills (anthropic-docx, today, visual-explainer) to add
  HWP format support to the project's data pipelines. Use when the user asks to
  "HWP pipeline", "HWP 파이프라인", "generate HWP report", "HWP 보고서 생성",
  "batch HWP processing", "HWP 일괄 처리", "rhwp-pipeline", "HWP report
  workflow", or wants end-to-end HWP document processing integrated with
  existing pipelines. Do NOT use for individual conversion (use rhwp-converter).
  Do NOT use for viewing (use rhwp-viewer). Do NOT use for debugging (use
  rhwp-debug). Do NOT use for setup (use rhwp-setup).
metadata:
  author: "thaki"
  version: "1.0.0"
