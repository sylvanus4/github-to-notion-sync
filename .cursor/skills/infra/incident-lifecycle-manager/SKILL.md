---
description: Full incident lifecycle: Slack alert → war-room channel → runbook retrieval from Cognee KG → customer notice draft → resolution → auto-postmortem → improvement item extraction → Notion tracking. Use when "incident lifecycle", "인시던트 관리", "full incident pipeline", "war room". Do NOT use for code debugging (use diagnose), security audit (use security-expert). Korean triggers: "인시던트 관리", "인시던트 라이프사이클", "전체 인시던트 파이프라인".
---
