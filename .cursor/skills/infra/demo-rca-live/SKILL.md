...
| 항목 | 확인 방법 |
|------|-----------|
| VPN 연결 | `curl -s -o /dev/null -w '%{http_code}' https://suite-demo.thakicloud.net/desktop` (200 확인) |
| kubectl 컨텍스트 | `kubectl config use-context tkai-demo-kubernetes-admin@tk-ai-demo-cluster` |
| DB 포트포워딩 | `kubectl -n postgresql port-forward svc/postgresql 15432:5432 &` |
| 로그인 정보 | `.secrets/infra-runbooks/Demo-웹-로그인.md` 참조 |
...
```bash
kubectl config use-context tkai-demo-kubernetes-admin@tk-ai-demo-cluster
```
...
```bash
# 기존 포트포워딩 확인
lsof -i :15432 2>/dev/null && echo "Port 15432 already in use" || kubectl -n postgresql port-forward svc/postgresql 15432:5432 &
```
...
SELECT type, id::text, name, status, created::text, stuck_duration::text FROM (
  ...
  WHERE status IN ('creating', 'starting') AND deleted_at IS NULL
    AND created_at < EXTRACT(EPOCH FROM NOW() - INTERVAL '15 minutes')
  UNION ALL
