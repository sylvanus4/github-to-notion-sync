...
version: 1.0.0
description: >
  Multi-channel feedback pipeline: ingests from app stores, surveys, support tickets,
  social media, NPS, and Slack; classifies by topic and sentiment; scores with RICE
  framework; synthesizes themes; generates executive .docx report with visual dashboard.
  Extends feedback-miner with automated RICE prioritization and executive deliverables.
  Use when the user asks to "process customer feedback", "feedback report",
  "multi-channel feedback analysis", "customer-feedback-processor", "RICE feedback
  prioritization", "고객 피드백 처리", "다채널 피드백 분석", "피드백 리포트 생성",
  "피드백 프로세서", or wants feedback-to-executive-report pipeline. Do NOT use for
  basic feedback mining (use feedback-miner). Do NOT use for financial sentiment
  (use alphaear-sentiment). Do NOT use for survey design (use kwp-design-user-research).
  Do NOT use for support ticket response (use kwp-customer-support-response-drafting).
tags: [feedback, customer, RICE, sentiment, multi-channel, executive-report, monetizable]
triggers:
  - "process customer feedback"
  - "customer feedback report"
  - "multi-channel feedback analysis"
  - "customer-feedback-processor"
  - "RICE feedback prioritization"
  - "executive feedback report"
  - "feedback-to-report pipeline"
  - "고객 피드백 처리"
  - "다채널 피드백 분석"
  - "피드백 리포트 생성"
  - "RICE 우선순위 피드백"
  ...
  - "고객 목소리 보고서"
do_not_use:
  - "For basic feedback mining without exec report (use feedback-miner)"
  - "For sentiment scoring of financial text (use alphaear-sentiment)"
