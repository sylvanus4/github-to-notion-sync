...
version: 1.0.0
description: >
  Meeting-to-proposal pipeline that ingests meeting transcripts or notes, extracts
  client requirements and project scope, generates a professional proposal and Statement
  of Work (SOW) as .docx with pricing tables, timelines, deliverables, and terms.
  Composes meeting-digest for transcript analysis and anthropic-docx for document
  generation. Use when the user asks to "generate proposal from meeting", "create SOW",
  "proposal-sow-generator", "meeting to proposal", "write proposal", "generate
  statement of work", "제안서 생성", "SOW 작성", "회의록에서 제안서", "미팅 기반 제안서",
  "작업 명세서", "프로포절 생성", or wants to convert meeting context into a professional
  proposal and SOW document package. Do NOT use for meeting summary without proposal
  intent (use meeting-digest). Do NOT use for NDA drafting (use pm-toolkit). Do NOT use
  for pricing strategy design (use marketing-sales-playbook). Do NOT use for contract
  review (use kwp-legal-contract-review).
tags: [proposal, SOW, meeting, document-generation, sales, monetizable]
triggers:
  - "generate proposal"
  - "create SOW"
  ...
  - "견적서 작성"
do_not_use:
  - "For meeting summary without proposal intent (use meeting-digest)"
  - "For NDA or legal document drafting (use pm-toolkit)"
  - "For pricing strategy without proposal context (use marketing-sales-playbook)"
  - "For contract review (use kwp-legal-contract-review)"
  - "For call preparation without proposal output (use kwp-sales-call-prep)"
composes:
  - meeting-digest
  - anthropic-docx
