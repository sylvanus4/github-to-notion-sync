...
version: 1.0.0
description: >
  Unified data analysis entry point that routes user questions to the appropriate
  kwp-data-* skill, supports an interactive evaluator-optimizer analysis loop,
  integrates experiment design (A/B testing, cohort analysis), and produces
  self-contained HTML dashboards as default output.
tags: [data, analysis, visualization, dashboard, experiment, A/B-test, orchestrator]
triggers:
  - "analyze this data"
  - "data analysis"
  - "upload and analyze"
  ...
  - "이 데이터 분석해줘"
do_not_use:
  - "For individual SQL query generation only (use kwp-data-sql-queries)"
  - "For chart type selection only (use kwp-data-data-visualization)"
  - "For data validation only (use kwp-data-data-validation)"
  - "For financial stock analysis (use daily-stock-check or today)"
  - "For general web research (use parallel-web-search)"
  - "For PM-specific cohort/A/B analysis in product context (use pm-data-analytics)"
composes:
  - kwp-data-data-exploration
