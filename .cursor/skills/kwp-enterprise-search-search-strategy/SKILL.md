---
name: kwp-enterprise-search-search-strategy
description: Query decomposition and multi-source search orchestration. Breaks natural language questions into targeted searches per source, translates queries into source-specific syntax, ranks results
  by relevance, and handles ambiguity and fallback strategies. Do NOT use for tasks outside the enterprise domain.
metadata:
...
Linear:  text search "API migration" in relevant workspace
```
...
**Filter mapping:**
| Enterprise filter | Linear parameter |
|------------------|----------------|
...
[User query]
     ↓ decompose
[Slack query] [Gmail query] [Google Drive query] [Wiki query] [Linear query]
     ↓            ↓            ↓              ↓            ↓
  (parallel execution)
     ↓
