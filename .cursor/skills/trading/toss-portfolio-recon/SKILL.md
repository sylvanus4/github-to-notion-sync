...
description: >-
  ...
  missed opportunities, signal mismatches, and allocation drift.
  Use when the user asks to "reconcile portfolio", "check drift", "portfolio recon",
  "포트폴리오 대조", "보유 종목 비교", "toss recon", "signal vs holdings",
  or when invoked by /tossinvest-recon command.
  Do NOT use for risk scoring (use toss-risk-monitor).
  Do NOT use for trade journal (use toss-trade-journal).
...
triggers:
  ...
  - 드리프트 체크
  - 파이프라인 보유 비교
tags: [trading, reconciliation, portfolio, toss-securities, pipeline]
metadata:
  author: "thaki"
  version: "1.0.0"
  category: "trading"
