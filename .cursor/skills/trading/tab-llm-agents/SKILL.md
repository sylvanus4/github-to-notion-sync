---
description: "Run LLM agent pipeline via POST /llm-agents/run and refresh macro data via POST /llm-agents/macro/refresh. Use when running LLM agents, 'LLM 에이전트 실행', 'tab-llm-agents'. Do NOT use for GenAI features (use tab-genai-features), stock price sync (use tab-stock-sync), or daily stock check (use daily-stock-check)."
---
