---
name: trading-edge-candidate-agent
description: >-
  Generate and prioritize US equity long-side edge research tickets from EOD observations,
  then export pipeline-ready candidate specs for trade-strategy-pipeline Phase I.
  Use when the user asks to "convert hypotheses to research tickets", "discover edge candidates",
  "export strategy.yaml", "validate edge-finder-candidate", "에디지 후보 생성", "리서치 티켓 변환", or wants to turn anomalies into reproducible research tickets.
  Do NOT use for daily stock signals (use daily-stock-check). Do NOT use for AlphaEar signal tracking (use alphaear-signal-tracker).
metadata:
  author: tradermonty
  version: "1.0.0"
  category: research
...
1. Run auto-detection from EOD OHLCV:
   - `.cursor/skills/trading-edge-candidate-agent/scripts/auto_detect_candidates.py`
...
4. Export candidate artifacts with `.cursor/skills/trading-edge-candidate-agent/scripts/export_candidate.py`.
5. Validate interface and Phase I constraints with `.cursor/skills/trading-edge-candidate-agent/scripts/validate_candidate.py`.
...
```bash
python3 .cursor/skills/trading-edge-candidate-agent/scripts/auto_detect_candidates.py \
  --ohlcv /path/to/ohlcv.parquet \
  --output-dir outputs/reports/trading/edge_candidate_auto \
  --top-n 10 \
...
```bash
python3 .cursor/skills/trading-edge-candidate-agent/scripts/export_candidate.py \
...
```bash
python3 .cursor/skills/trading-edge-candidate-agent/scripts/validate_candidate.py \
...
### `.cursor/skills/trading-edge-candidate-agent/scripts/export_candidate.py`
Generate `strategies/<candidate_id>/strategy.yaml` and `metadata.json` from a research ticket YAML.

### `.cursor/skills/trading-edge-candidate-agent/scripts/validate_candidate.py`
...
### Example 1: Auto-detect and export
**User:** "Run daily edge detection on my OHLCV parquet and export top 10 candidates to the pipeline."
**Action:** Runs `auto_detect_candidates.py` with `--ohlcv`, `--top-n 10`, `--export-strategies-dir`; validates and exports to `trade-strategy-pipeline/strategies`.
**Output:** Daily report, anomalies, watchlist, and exportable `strategy.yaml` + `metadata.json` in pipeline strategies dir.

### Example 2: Export existing ticket
**User:** "Convert this research ticket to a pipeline-ready candidate and validate it."
**Action:** Runs `export_candidate.py` from ticket YAML, then `validate_candidate.py` with `--pipeline-root` and `--stage phase1`.
**Output:** `strategies/<candidate_id>/` with `strategy.yaml`, `metadata.json`, and validation pass/fail report.

### Example 3: Interface-only validation
**User:** "Check if this strategy.yaml meets the edge-finder-candidate/v1 contract."
**Action:** Runs `validate_candidate.py --strategy path/to/strategy.yaml` without pipeline root.
**Output:** Pass/fail on interface contract (entry families, risk bounds, schema compliance).

## Error Handling

| Error | Action |
|-------|--------|
| Validation fails: unsupported entry family | Use only `pivot_breakout`+`vcp_detection` or `gap_up_continuation`+`gap_up_detection` for v1 export |
| Folder name ≠ strategy id | Ensure `strategies/<id>/strategy.yaml` has `id` matching folder name exactly |
| PyYAML missing | Install: `pip install PyYAML` |
| Pipeline schema validation fails | Run `--dry-run` in pipeline first; fix `StrategySpec` or stage rules per `pipeline_if_v1.md` |
