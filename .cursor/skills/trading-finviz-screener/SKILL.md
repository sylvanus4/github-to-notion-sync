---
name: trading-finviz-screener
description: >-
  Build and open FinViz screener URLs from natural language requests. Use when the user asks to "screen stocks", "find stocks matching criteria", "FinViz screener", "스크리너", "FinViz로 주식 필터", "高配当で成長している小型株を探したい", "Find oversold large caps with high ROE", or wants to filter by fundamentals or technicals. Supports Japanese and English input. No API key required for public screener. Do NOT use for daily stock signals (use daily-stock-check). Do NOT use for AlphaEar stock search (use alphaear-stock).
metadata:
  author: tradermonty
  version: "1.0.0"
  category: execution
...
```bash
python3 .cursor/skills/trading-finviz-screener/scripts/open_finviz_screener.py \
...
### Example 1: High-dividend growth
**User:** "Find oversold large caps with high ROE and dividend above 3%."
**Action:** Maps to `ta_rsi_os30`, `cap_large`, `fa_roe_o15`, `fa_div_o3`; builds URL, opens Chrome with filters applied.
**Output:** FinViz screener URL opens in browser; summary of filters and suggested next steps (e.g., sort by dividend yield).

### Example 2: Theme + sub-theme drill-down
**User:** "Show me AI cloud and compute stocks, mid cap and above."
**Action:** Uses `--themes artificialintelligence --subthemes aicloud,aicompute --filters cap_midover`.
**Output:** FinViz page filtered by AI theme, cloud/compute sub-themes, and cap filter; reports URL and mode (Elite/Public).

### Example 3: Japanese request
**User:** "高配当で成長している小型株を探したい"
**Action:** Maps to small cap, high dividend, earnings growth filters; presents table for confirmation, then executes.
**Output:** Screener URL with `cap_small`, `fa_div_o3`, growth filters applied; opens in browser.

## Error Handling

| Error | Action |
|-------|--------|
| Invalid filter code | Check `references/finviz_screener_filters.md`; avoid `theme_*` / `subtheme_*` in `--filters` (use `--themes` / `--subthemes`) |
| Chrome fails to open | Script falls back to default browser; verify Chrome path or use `--url-only` and open manually |
| No filters/themes/subthemes provided | At least one of `--filters`, `--themes`, or `--subthemes` is required |
| Rate limit (public mode) | Wait and retry; consider FINVIZ Elite for faster execution |
