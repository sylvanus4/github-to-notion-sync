...
description: >-
  ...
  templates, outlines, word counts, and bases via the CLI. Use when the
  user asks to manage tags, list tasks, edit properties, apply templates,
  view note outline, check word count, or work with Obsidian Bases. Do
  NOT use for file CRUD (use obsidian-files), daily notes
  (use obsidian-daily), search (use obsidian-search), plugin management
  (use obsidian-admin), or developer tools (use obsidian-dev).
  Korean triggers: "옵시디언 태그", "태그 관리", "옵시디언 작업", "할일 목록",
  "프로퍼티", "프론트매터", "옵시디언 템플릿", "노트 개요", "단어 수",
  "옵시디언 베이스".
metadata:
  version: "1.0.0"
