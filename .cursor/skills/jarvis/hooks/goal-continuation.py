#!/usr/bin/env python3
"""Jarvis Goal Mode — CLI utility for goal state evaluation (Cursor edition).

Reads active goal state, runs check_cmd criteria, checks budget, and prints
a JSON status report. Used by the agent during Goal Mode iterations.
...
Exit codes:
  0 = pursuing (continue)
  1 = terminal (achieved/budget-limited/unmet — stop)
...
GOALS_DIR = REPO_ROOT / ".cursor/skills/jarvis/state/goals"
TIMEOUT_SEC = 15
...
def build_continuation_prompt(g: dict, met: list, remaining: list) -> str:
    ...
    state_path = f".cursor/skills/jarvis/state/goals/{g.get('id', 'unknown')}.json"
