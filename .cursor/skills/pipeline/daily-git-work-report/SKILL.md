...
description: >-
  Summarize yesterday and today's GitHub commits plus agent sessions across all
  5 managed repositories into a structured Korean report with remaining-work
  status. Posts to Slack and saves to disk. Use when the user asks to "daily
  work report", "yesterday today work summary", "git work report", "어제 오늘
  작업 정리", "일일 작업 보고", "깃 작업 요약", "작업 내역 정리", "어제 오늘
  작업한 내용 정리", "daily git work report", or wants a consolidated summary
  of recent development activity across projects. Do NOT use for weekly status
  reports (use weekly-status-report), sprint-level GitHub platform events like
...
| research | `/Users/hanhyojung/thaki/research` | `main` |
