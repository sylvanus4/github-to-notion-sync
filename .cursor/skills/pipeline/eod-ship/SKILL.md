...
**Thread Reply — Git Work History (어제+오늘 작업 내역)**: Post a second thread reply showing **yesterday and today's commit history** across all 5 repos with **local file paths**. This helps the user orient where work was done locally.
...
Parse the output to build a per-repo commit list with changed file paths. For each commit, the user needs to know `PROJECT_PATH` (absolute local path) plus the relative file path within the repo.
...
**Thread message template** — Post as a thread reply (split into multiple messages if > 4000 chars):
...
```json
{
  "channel_id": "C0AA8NT4T8T",
  "thread_ts": "<thread_ts from main message>",
  "message": "<git work history mrkdwn>"
}
```

If the thread reply fails, log a warning and continue.
