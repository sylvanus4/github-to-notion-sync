...
description: >-
  ...
  Supports US/HK equities (yfinance), China A-shares (tushare), and crypto (OKX)
  with automatic source detection from symbol format.
  Use when the user asks to "fetch stock data via vibe", "get crypto OHLCV",
  "vibe-trading data", "vibe market data", "바이브 시세 조회",
  "바이브 트레이딩 데이터", or needs OHLCV data through the Vibe-Trading MCP.
  Do NOT use for Yahoo Finance data via the project's own weekly-stock-update.
  Do NOT use for AlphaEar stock search (use alphaear-stock).
  Do NOT use for setup (use vibe-trading-setup).
tags: [vibe-trading, data, ohlcv, market-data]
version: 1.0.0
---
