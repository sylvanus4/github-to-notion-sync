...
description: >-
  Install Vibe-Trading, configure environment variables, register the MCP server
  in Cursor, and run health checks to verify connectivity.
  Use when the user asks to "install vibe-trading", "setup vibe trading",
  "configure vibe-trading", "vibe-trading-setup", "바이브 트레이딩 설치",
  "바이브 트레이딩 설정", or needs to set up the Vibe-Trading MCP integration.
  Do NOT use for running analysis (use vibe-trading-orchestrator).
  Do NOT use for MCP tool reference (use vibe-trading-mcp).
  Do NOT use for data fetching (use vibe-trading-data).
tags: [vibe-trading, setup, mcp, installation]
version: 1.0.0
---
