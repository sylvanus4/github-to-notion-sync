...
- Prompt: Read and follow `.cursor/skills/kwp-engineering-incident-response/SKILL.md`. Triage this incident: classify severity, assess blast radius, identify affected services, recommend immediate mitigation steps. Incident: `{user_description}`.
- Expected output: severity, impact assessment, affected services, mitigation steps

**Agent 2: Diagnose (Root Cause)**
- `subagent_type: generalPurpose`, `model: fast`, `readonly: true`
- Prompt: Read and follow `.cursor/skills/diagnose/SKILL.md`. Run with `--no-fix`. Analyze: `{user_description}`. Return root cause hypothesis, confidence level, and evidence chain.
...
- Prompt: Read and follow `.cursor/skills/test-suite/SKILL.md`. For the fix applied to `{files}`, generate regression tests that:
...
- Prompt: Read and follow `.cursor/skills/kwp-customer-support-knowledge-management/SKILL.md`. Create a KB article for this incident:
...
- `subagent_type: generalPurpose`, `model: fast`, `readonly: true`
- Prompt: Read and follow `.cursor/skills/sre-devops-expert/SKILL.md`. Based on this incident (root cause: `{root_cause}`), recommend:
...
- `subagent_type: generalPurpose`, `model: fast`
- Prompt: Read and follow `.cursor/skills/technical-writer/SKILL.md`. Generate a blameless post-mortem document using this template:
