...
description: >-
  Notion CRUD for Weekly Release and Hotfix Queue databases. Create weekly release
  pages, update item statuses, query by status/app/date, and manage hotfix queue
  entries. Use when the user asks to "create release board", "update release status",
  "query release items", "릴리즈 보드 생성", "릴리즈 상태 업데이트", "release-notion-board",
  or any Notion CRUD for release operations. Do NOT use for general Notion page
  creation (use md-to-notion), meeting sync (use notion-meeting-sync), or non-release
  database operations (use Notion MCP directly).
metadata:
  version: "1.0.0"
  category: "execution"
  author: "thaki"
