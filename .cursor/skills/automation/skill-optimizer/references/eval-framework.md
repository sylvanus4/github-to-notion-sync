...
Help the agent do something it cannot do (or cannot do reliably) without the skill. Examples: Notion publishing workflows, structured PRD generation, policy-compliant copy.
...
Capture a workflow the agent could approximate but the skill encodes specific process, order, or formatting requirements. Examples: document quality rubric, design sync matrices.
