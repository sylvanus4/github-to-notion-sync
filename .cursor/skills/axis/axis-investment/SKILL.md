...
description: >-
  ...
  management, trading signals, and investment content creation. Thin orchestrator
  wrapping the existing `today` pipeline and adding content distribution, broker
  operations, and EOD summary. Use when the user asks for "axis investment",
  "investment axis", "투자 축", "market axis", "axis-investment", or wants the
  full investment intelligence cycle including content generation.
  Do NOT use for running the today pipeline directly (use today). Do NOT use
  for weekly price sync only (use weekly-stock-update). Do NOT use for broker
  operations only (use toss-ops-orchestrator or kis-team directly).
metadata:
  author: thaki
  version: "1.0.0"
