...
description: >
  ...
  components), state management (decouple state from UI, generic context,
  lift state), implementation patterns (explicit variants, children over
  render props), and React 19 API changes.
  Adapted from Vercel's composition-patterns skill.
triggers:
  - composition pattern
  - compound component
...
boundaries:
  ...
  - Do NOT use for general frontend code review (use frontend-expert)
  - Do NOT use for FSD architecture scaffolding (use fsd-development)
  - Do NOT use for design system token compliance (use design-qa-checklist)
  - Do NOT use for UI code audit (use web-interface-audit)
  - Do NOT use for Tailwind design system setup (use tailwind-design-system)
  - Do NOT use for Next.js App Router patterns (use nextjs-best-practices)
---
