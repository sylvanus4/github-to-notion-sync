...
name: ux-copy-audit
description: >-
  Scan codebase for UI text strings; detect policy violations, inconsistent copy,
  missing translations, hardcoded strings, and tone misalignment. Cross-reference
  UX writing policy and design system guidance; produce an audit report with fixes.
  Korean triggers: "UX 카피 감사", "ux copy audit", "문구 감사", "UI 텍스트 스캔",
  "copy audit", "문구 일관성 스캔", "UI 문자열", "하드코딩 문구". Do NOT use for
  generating new UX copy from scratch only (use ux-writing-agent), policy-to-UI batch
  generation (use policy-text-generator), or WCAG-only audits without copy scope
