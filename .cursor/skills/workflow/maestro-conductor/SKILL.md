...
description: >-
  Long-running mission orchestrator with plan-approve-execute lifecycle, durable
  cross-session state, milestone-gated phases, handoff brief generation, and
  checkpoint save/restore. Extends mission-control with persistent multi-session
  mission state that survives context window resets.
  Use when the user asks to "create a mission", "long-running task", "multi-session
  project", "plan-approve-execute", "mission orchestrator", "handoff brief",
  "checkpoint save", "restore mission", "미션 생성", "장기 미션", "멀티세션 프로젝트",
  "핸드오프 브리프", "체크포인트", "미션 복원", "계획-승인-실행", "maestro-conductor",
  or wants durable cross-session orchestration with approval gates.
  Do NOT use for single-session task orchestration (use mission-control).
  Do NOT use for simple todo tracking (use TodoWrite).
  Do NOT use for code review pipelines (use deep-review or ship).
  Do NOT use for agent memory (use icarus-memory-fabric or memkraft).
  Korean triggers: "미션", "장기 프로젝트", "핸드오프", "체크포인트", "마에스트로".
metadata:
  author: "thaki"
  version: "1.0.0"
  category: "workflow"
