...
| **데이터 동기화** | 필터링 | `--sprint-filter` 옵션으로 특정 스프린트만 동기화 | 예: `25-12-Sprint1` |
...
## 2. Sprint PR Review Check (PR 리뷰 체크)
...
| **사용자 매핑** | GitHub → Notion | `field_mappings.yml`의 매핑 사용 | |
...
## 4. Sprint Summary Sync (AI 스프린트 요약)
...
| **Notion 페이지** | 위치 | SprintChecker 페이지 하위 | `sprint_checker_parent_id` |
| | 구조 | 헤더 + 통계 + 사용자별 섹션 | |
...
| **Notion 페이지** | 위치 | DailyScrum 페이지 하위 | `daily_scrum_parent_id` |
...
GitHub 사용자와 Notion 사용자 간 매핑은 `config/field_mappings.yml`에서 관리합니다.
